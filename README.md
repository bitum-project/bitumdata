...
- [bitumdata](#bitumdata)
  ...
  - [Building](#building)
    ...
    - [Building bitumdata with Go](#building-bitumdata-with-go)
  ...
  - [System Hardware Requirements](#system-hardware-requirements)
    ...
    - [bitumdata and PostgreSQL on same host](#bitumdata-and-postgresql-on-same-host)
  - [bitumdata Daemon](#bitumdata-daemon)
    - [Block Explorer](#block-explorer)
  - [APIs](#apis)
    - [Insight API](#insight-api)
    - [bitumdata API](#bitumdata-api)
...
```none
../bitumdata              The bitumdata daemon.
├── api                 Package blockdata implements bitumdata's own HTTP API.
│   ├── insight         Package insight implements the Insight API.
│   └── types           Package types includes the exported structures used by
|                         the bitumdata and Insight APIs.
...
│   ├── cache           Package cache provides a caching layer that is used by bitumpg.
│   ├── dbtypes         Package dbtypes with common data types.
│   ├── bitumpg           Package bitumpg providing PostgreSQL backend.
│   └── bitumsqlite       Package bitumsqlite providing SQLite backend.
├── bitumrates            Package bitumrates defines a gRPC protobuf service for
...
│                         data to multiple clients like bitumdata.
...
├── notification        Package notification manages bitumd notifications, and
...
├── version             Package version describes the bitumdata version.
...
documentation](docs/docker.md) for more information. The supported bitumdata build
instructions are described below.

## Building

The bitumdata build process comprises two general steps:

1. Bundle the static web page assets with Webpack (via the `npm` tool).
2. Build the `bitumdata` executable from the Go source files.
...
- Clone the bitumdata repository. It is conventional to put it under `GOPATH`, but
  this is no longer necessary with go module. For example:

  ```sh
  git clone https://github.com/bitum-project/bitumdata $HOME/go-work/github/bitum-project/bitumdata
  ```
...
**Beware:** For the v4 bitumdata module, the executable generated by `go build`
may be named "v4" instead of "bitumdata". This
...
`bitumdata version 4.0.0-pre+dev (Go version go1.12.1)`.  However, it may be
...
folders to ensure that _the user running bitumdata_ is able to access these paths.

The "public" and "views" folders _must_ be in the same folder as the `bitumdata`
...
```sh
cd $HOME/go-work/github/bitum-project/bitumdata
...
Next, build `bitumdata` and bundle the web assets:
...
No special actions are required. Simply start the new bitumdata and automatic
database schema upgrades and table data patches will begin.

### From v2.x or earlier

The database scheme change from bitumdata v2.x to v3.x does not permit an
automatic migration. The tables must be rebuilt from scratch:

1. Drop the old bitumdata database, and create a new empty bitumdata database.

   ```sql
   -- Drop the old database.
   DROP DATABASE bitumdata;

   -- Create a new database with the same "pguser" set in the bitumdata.conf.
   CREATE DATABASE bitumdata OWNER bitumdata;
   ```

2. Delete the bitumdata data folder (i.e. corresponding to the `datadir` setting).
   By default, `datadir` is in `{appdata}/data`:

   - Linux: `~/.bitumdata/data`
   - Mac: `~/Library/Application Support/Bitumdata/data`
   - Windows: `C:\Users\<your-username>\AppData\Local\Bitumdata\data` (`%localappdata%\Bitumdata\data`)

3. With bitumd synchronized to the network's best block, start bitumdata to begin
...
subtract 1.5-2GB from your system RAM so bitumdata itself will have plenty of
...
```sh
cp sample-bitumdata.conf ~/.bitumdata/bitumdata.conf
```

Then edit bitumdata.conf with your bitumd RPC settings. See the output of `bitumdata
...
For a list of the recognized environment variables, run `bitumdata --help`.

### Indexing the Blockchain

If bitumdata has not previously been run with the PostgreSQL database backend, it
is necessary to perform a bulk import of blockchain data and generate table
indexes. _This will be done automatically by `bitumdata`_ on a fresh startup.

Note that bitumdata requires that
[bitumd](https://docs.bitum.io/getting-started/user-guides/bitumd-setup/) is
running with some optional indexes enabled. By default, these indexes are not
turned on when bitumd is installed. To enable them, set the following in
bitumd.conf:
...
If these parameters are not set, bitumdata will be unable to retrieve transaction
details and perform address searches, and will exit with an error mentioning
these indexes.

### Starting bitumdata

Launch the bitumdata daemon and allow the databases to process new blocks.
...
On subsequent launches, only blocks new to bitumdata are processed.

```sh
./bitumdata    # don't forget to configure bitumdata.conf in the appdata folder!
```

Unlike bitumdata.conf, which must be placed in the `appdata` folder or explicitly
...
By default, on bitumdata startup, a syncing status page is the only page available
...
with a value on the range `[2,5000]`, all bitumdata pages will be active on
startup if the number of remaining blocks to process are less than the specified
value.

For example, if `sync-status-limit` is set to 1000, all bitumdata pages will be
...
than 1000 especially if your machine struggles handling bitumdata normal load.
...
### bitumdata only (PostgreSQL on other host)
...
### bitumdata and PostgreSQL on same host

These specifications assume bitumdata and postgres are running on the same machine.
...
After bitumdata syncs with the blockchain server via RPC, by default it will begin
...
Note that while bitumdata can be started with HTTPS support, it is recommended to
employ a reverse proxy such as Nginx ("engine x"). See sample-nginx.conf for an
example Nginx configuration.

## APIs

The bitumdata block explorer is exposed by two APIs: a Bitum implementation of
...
bitumdata API uses the path prefix `/api`.
...
The bitumdata API is a REST API accessible via HTTP. To call the bitumdata API, use
the `/api` path prefix.
...
| Verbose block result | `/block/best/verbose`               | `bitumjson.GetBlockVerboseResult`       |

| Block X (block index) | Path                  | Type                                  |
| --------------------- | --------------------- | ------------------------------------- |
| Summary               | `/block/X`            | `types.BlockDataBasic`                |
| Stake info            | `/block/X/pos`        | `types.StakeInfoExtended`             |
| Header                | `/block/X/header`     | `bitumjson.GetBlockHeaderVerboseResult` |
...
| Verbose block result  | `/block/X/verbose`    | `bitumjson.GetBlockVerboseResult`       |

| Block H (block hash) | Path                       | Type                                  |
| -------------------- | -------------------------- | ------------------------------------- |
| Summary              | `/block/hash/H`            | `types.BlockDataBasic`                |
| Stake info           | `/block/hash/H/pos`        | `types.StakeInfoExtended`             |
| Header               | `/block/hash/H/header`     | `bitumjson.GetBlockHeaderVerboseResult` |
...
| Verbose block result | `/block/hash/H/verbose`    | `bitumjson.GetBlockVerboseResult`       |
...
| Current sdiff separately               | `/stake/diff/current`   | `bitumjson.GetStakeDifficultyResult` |
| Estimates separately                   | `/stake/diff/estimates` | `bitumjson.EstimateStakeDiffResult`  |
...
| --------------------------------- | --------------------- | --------------------------- |
| The current agenda and its status | `/stake/vote/info`    | `bitumjson.GetVoteInfoResult` |
...
to keep in mind that the mempool in your node (bitumd) is not likely to be exactly
the same as other nodes' mempool. Also, your mempool is cleared out when you
shutdown bitumd. So, if you have recently (e.g. after the start of the current
ticket price window) started bitumd, your mempool _will_ be missing transactions
...
of the bitumdata daemon, but may be called alone with rebuilddb.

### rebuilddb2

`rebuilddb2` is a CLI app used for maintenance of bitumdata's `bitumpg` database
(a.k.a. DB v2) that uses PostgreSQL to store a nearly complete record of the
Bitum blockchain data. This functionality is included in the startup of the
bitumdata daemon, but may be called alone with rebuilddb2. See the
...
From the bitumdata root directory, run the following command to install the node
modules.
...
The core functionality of bitumdata is server-side rendered in Go and designed to
...
The core functionality of bitumdata should perform well in low power device / high
...
converting from standard Bitum data types (e.g. `wire.MsgBlock`) are also
provided.
...
`github.com/bitum-project/bitumd/database` with an ffldb storage backend from
`github.com/bitum-project/bitumd/database/ffldb`. It also makes use of the `stake.Node`
type from `github.com/bitum-project/bitumd/blockchain/stake`. The `ChainMonitor` type
...
`bitumutil.Tx`, `bitumutil.Block`, `chainhash.Hash`, and others.

## Internal-use Packages

Some packages are currently designed only for internal
use by other bitumdata packages, but may be of general value in
the future.
...
- The `ChainDB` type, which is the primary exported type from `bitumpg`, providing
...
- The `WiredDB` type, intended to satisfy the `DataSourceLite` interface used by
  the bitumdata app's API. The block header is not stored in the DB, so a RPC
...
See the GitHub [issue trackers](https://github.com/bitum-project/bitumdata/issues) and
the [project milestones](https://github.com/bitum-project/bitumdata/milestones).
...
5. Create a [pull request](https://github.com/bitum-project/bitumdata/compare).
