...
const (
	...
	// getrawtransaction RPC call to bitumd.
...
func NewInsightApi(client *rpcclient.Client, blockData *bitumpg.ChainDBRPC, params *chaincfg.Params,
...
func (iapi *InsightApi) getTransaction(w http.ResponseWriter, r *http.Request) {
	...
	txsOld := []*bitumjson.TxRawResult{txOld}
...
func (iapi *InsightApi) getBlockSummary(w http.ResponseWriter, r *http.Request) {
	...
	blockSummary := []*bitumjson.GetBlockVerboseResult{block}
	blockInsight, err := iapi.BitumToInsightBlock(blockSummary)
	if err != nil {
		...
		FUNDING_TX_DUPLICATE_CHECK:
			for _, f := range addressOuts.Outpoints {
				...
				txnOutput := apitypes.AddressTxnOutput{
					...
					Amount:        bitumutil.Amount(fundingTx.Tx.TxOut[f.Index].Value).ToCoin(),
...
func (iapi *InsightApi) getTransactions(w http.ResponseWriter, r *http.Request) {
	...
	if addrerr == nil {
		address := addresses[0]
		// Validate Address
		_, err := bitumutil.DecodeAddress(address)
		if err != nil {
...
func (iapi *InsightApi) getAddressesTxn(w http.ResponseWriter, r *http.Request) {
	...
	txsNew, err := iapi.BitumToInsightTxns(txsOld, noAsm, noScriptSig, noSpent)
	if err != nil {
...
func (iapi *InsightApi) getAddressInfo(w http.ResponseWriter, r *http.Request) {
	...
	SPENDING_TX_DUPLICATE_CHECK:
		for _, f := range addressOuts.PrevOuts {
			...
			// because vin:i valuein is not reliable from bitumd at present.
			// TODO(chappjc): bitumd should be OK now. Recheck this.
	...
	addressInfo := apitypes.InsightAddressInfo{
		...
		TotalReceived:            bitumutil.Amount(balance.TotalSpent + balance.TotalUnspent).ToCoin(),
		TotalSent:                bitumutil.Amount(balance.TotalSpent).ToCoin(),
		Balance:                  bitumutil.Amount(balance.TotalUnspent).ToCoin(),
		TxAppearances:            int64(confirmedTxCount),
		UnconfirmedBalance:       bitumutil.Amount(unconfirmedBalanceSat).ToCoin(),
...
func (iapi *InsightApi) getEstimateFee(w http.ResponseWriter, r *http.Request) {
	...
	// A better solution would be a call to the BITUMD RPC "estimatefee" endpoint
