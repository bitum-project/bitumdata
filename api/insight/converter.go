...
// TxConverter converts bitumd-tx to insight tx
func (iapi *InsightApi) TxConverter(txs []*bitumjson.TxRawResult) ([]apitypes.InsightTx, error) {
	return iapi.BitumToInsightTxns(txs, false, false, false)
}

// BitumToInsightTxns converts a bitumjson TxRawResult to a InsightTx. The asm,
// scriptSig, and spending status may be skipped by setting the appropriate
// input arguments.
func (iapi *InsightApi) BitumToInsightTxns(txs []*bitumjson.TxRawResult, noAsm, noScriptSig, noSpent bool) ([]apitypes.InsightTx, error) {
	...
	for _, tx := range txs {
		...
		for vinID, vin := range tx.Vin {
			...
			if err == nil {
				if len(addresses) > 0 {
					// Update Vin due to BITUMD AMOUNTIN - START
					// NOTE THIS IS ONLY USEFUL FOR INPUT AMOUNTS THAT ARE NOT ALSO FROM MEMPOOL
					if tx.Confirmations == 0 {
						InsightVin.Value = bitumutil.Amount(value).ToCoin()
					}
					// Update Vin due to BITUMD AMOUNTIN - END
					InsightVin.Addr = addresses[0]
				}
			}
			bitumamt, _ := bitumutil.NewAmount(InsightVin.Value)
			InsightVin.ValueSat = int64(bitumamt)
		...
		bitumamt, _ := bitumutil.NewAmount(vOutSum)
		txNew.ValueOut = bitumamt.ToCoin()

		bitumamt, _ = bitumutil.NewAmount(vInSum)
		txNew.ValueIn = bitumamt.ToCoin()

		bitumamt, _ = bitumutil.NewAmount(txNew.ValueIn - txNew.ValueOut)
		txNew.Fees = bitumamt.ToCoin()
...
// BitumToInsightBlock converts a bitumjson.GetBlockVerboseResult to Insight block.
func (iapi *InsightApi) BitumToInsightBlock(inBlocks []*bitumjson.GetBlockVerboseResult) ([]*apitypes.InsightBlockResult, error) {
	RewardAtBlock := func(blocknum int64, voters uint16) float64 {
		...
		return bitumutil.Amount(work + stake + tax).ToCoin()
	}
