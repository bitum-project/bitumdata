...
func NewSocketServer(params *chaincfg.Params, txGetter txhelpers.RawTransactionGetter) (*SocketServer, error) {
	...
	server.On("connection", func(so socketio.Socket) {
		...
		so.On("subscribe", func(room string) {
			...
			if addr, err := bitumutil.DecodeAddress(room); err == nil {
...
// SendNewTx prepares a bitumd mempool tx for broadcast. This method satisfies
// notification.TxHandler and is registered as a handler in main.go.
func (soc *SocketServer) SendNewTx(rawTx *bitumjson.TxRawResult) error {
...
func (soc *SocketServer) sendNewTx(msgTx *wire.MsgTx, vouts []bitumjson.Vout) error {
