...
// much of the time, bitumdata will be using the types in bitumjson, but others are
// defined here
...
// of this type are NOT compatible with bitumd's txscript.ScriptClass values! Use
...
// class. If bitumd's txscript package changes its strings, this function may be
// unable to identify the types from bitumd.
...
// TxOut defines a bitum transaction output.
type TxOut struct {
	Value               float64      `json:"value"`
	Version             uint16       `json:"version"`
	ScriptPubKeyDecoded ScriptPubKey `json:"scriptPubKey"`
...
// TxIn defines a bitum transaction input.
type TxIn struct {
	// Non-witness
...
type AddressTxRaw struct {
	...
	Version       int32                `json:"version"`
	Locktime      uint32               `json:"locktime"`
	Vin           []bitumjson.VinPrevOut `json:"vin"`
	Vout          []Vout               `json:"vout"`
	Confirmations int64                `json:"confirmations"`
	BlockHash     string               `json:"blockhash"`
...
type AddressTotals struct {
	...
	CoinsSpent   float64 `json:"bitum_spent"`
	CoinsUnspent float64 `json:"bitum_unspent"`
}

// BlockDataWithTxType adds an array of TxRawWithTxType to
// bitumjson.GetBlockVerboseResult to include the stake transaction type
type BlockDataWithTxType struct {
	*bitumjson.GetBlockVerboseResult
	Votes   []TxRawWithVoteInfo
	Tickets []bitumjson.TxRawResult
	Revs    []bitumjson.TxRawResult
}

// TxRawWithVoteInfo adds the vote info to bitumjson.TxRawResult
type TxRawWithVoteInfo struct {
	bitumjson.TxRawResult
	VoteInfo VoteInfo
}

// TxRawWithTxType adds the stake transaction type to bitumjson.TxRawResult
...
type APIStatus struct {
	...
	BitumdataVersion  string `json:"bitumdata_version"`
	NetworkName     string `json:"network_name"`
}

// NewStatus is the constructor for a new Status.
func NewStatus(nodeHeight uint32, conns int64, apiVersion int, bitumdataVersion, netName string) *Status {
	return &Status{
		height:          nodeHeight,
		nodeConnections: conns,
		api: APIStatus{
			APIVersion:     apiVersion,
			BitumdataVersion: bitumdataVersion,
			NetworkName:    netName,
		},
	}
}
...
func (s *Status) API() APIStatus {
	...
	return APIStatus{
		...
		BitumdataVersion:  s.api.BitumdataVersion,
...
type BlockExplorerExtraInfo struct {
	...
	NextBlockSubsidy *bitumjson.GetBlockSubsidyResult `json:"next_block_subsidy"`
}
...
type StakeDiff struct {
	bitumjson.GetStakeDifficultyResult
	Estimates        bitumjson.EstimateStakeDiffResult `json:"estimates"`
...
type StakeInfoExtended struct {
	Hash             string               `json:"hash"`
	Feeinfo          bitumjson.FeeInfoBlock `json:"feeinfo"`
