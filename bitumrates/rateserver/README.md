...
RateServer is a central server for distributing Bitum exchange rate data to
any number of clients. It is particularly useful for BITUMData instances behind a
...
```
cd path/to/bitumdata/bitumrates/rateserver
go build
./rateserver
```

See [BITUMData's notes on Go](https://github.com/bitum-project/bitumdata#building-bitumdata-with-go-111)
...
### Using RateServer with BITUMData

To point BITUMData to RateServer, you must set BITUMData's `ratemaster` and
...
-c, --config=            Path to a custom configuration file.
    --appdir=            Path to application home directory. (~/.bitumrates)
