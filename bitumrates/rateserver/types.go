...
type RateClient interface {
	SendExchangeUpdate(*bitumrates.ExchangeRateUpdate) error
	Stream() GRPCStream
}
...
type GRPCStream interface {
	Send(*bitumrates.ExchangeRateUpdate) error
...
// SubscribeExchanges is a gRPC method defined in bitumrates/bitumrates.proto. It
// satisfies the BITUMRatesServer interface. The gRPC server will call this method
// when a client subscription is received.
func (server *RateServer) SubscribeExchanges(hello *bitumrates.ExchangeSubscription, stream bitumrates.BITUMRates_SubscribeExchangesServer) (err error) {
...
func (server *RateServer) ReallySubscribeExchanges(hello *bitumrates.ExchangeSubscription, stream GRPCStream) (err error) {
	...
	// Send Bitum exchanges.
	err = sendStateList(client, state.BitumBtc)
	if err != nil {
		return err
	}
	// Send Bitcoin-fiat indices.
	for token := range state.FiatIndices {
		err = client.SendExchangeUpdate(&bitumrates.ExchangeRateUpdate{
			Token:   token,
...
func (server *RateServer) addClient(stream GRPCStream, hello *bitumrates.ExchangeSubscription) (RateClient, StreamID) {
...
func makeExchangeRateUpdate(update *exchanges.ExchangeUpdate) *bitumrates.ExchangeRateUpdate {
	state := update.State
	protoUpdate := &bitumrates.ExchangeRateUpdate{
		Token:      update.Token,
		Price:      state.Price,
	...
	if state.Candlesticks != nil {
		protoUpdate.Candlesticks = make([]*bitumrates.ExchangeRateUpdate_Candlesticks, 0, len(state.Candlesticks))
		for bin, sticks := range state.Candlesticks {
			candlesticks := &bitumrates.ExchangeRateUpdate_Candlesticks{
				Bin:    string(bin),
				Sticks: make([]*bitumrates.ExchangeRateUpdate_Candlestick, 0, len(sticks)),
			}
			for _, stick := range sticks {
				candlesticks.Sticks = append(candlesticks.Sticks, &bitumrates.ExchangeRateUpdate_Candlestick{
					High:   stick.High,
					Low:    stick.Low,
					Open:   stick.Open,
					Close:  stick.Close,
	...
	if state.Depth != nil {
		depth := &bitumrates.ExchangeRateUpdate_DepthData{
			Time: state.Depth.Time,
			Bids: make([]*bitumrates.ExchangeRateUpdate_DepthPoint, 0, len(state.Depth.Bids)),
			Asks: make([]*bitumrates.ExchangeRateUpdate_DepthPoint, 0, len(state.Depth.Asks)),
		}
		for _, pt := range state.Depth.Bids {
			depth.Bids = append(depth.Bids, &bitumrates.ExchangeRateUpdate_DepthPoint{
				Quantity: pt.Quantity,
				Price:    pt.Price,
			})
		}
		for _, pt := range state.Depth.Asks {
			depth.Asks = append(depth.Asks, &bitumrates.ExchangeRateUpdate_DepthPoint{
				Quantity: pt.Quantity,
				Price:    pt.Price,
			})
		}
...
func (client *rateClient) SendExchangeUpdate(update *bitumrates.ExchangeRateUpdate) (err error) {
