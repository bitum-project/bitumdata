...
type BlockData struct {
	Header           bitumjson.GetBlockHeaderVerboseResult
	Connections      int32
	FeeInfo          bitumjson.FeeInfoBlock
	CurrentStakeDiff bitumjson.GetStakeDifficultyResult
	EstStakeDiff     bitumjson.EstimateStakeDiffResult
...
type Collector struct {
	mtx          sync.Mutex
	bitumdChainSvr *rpcclient.Client
	netParams    *chaincfg.Params
	stakeDB      *stakedb.StakeDatabase
}

// NewCollector creates a new Collector.
func NewCollector(bitumdChainSvr *rpcclient.Client, params *chaincfg.Params,
	stakeDB *stakedb.StakeDatabase) *Collector {
	return &Collector{
		bitumdChainSvr: bitumdChainSvr,
		netParams:    params,
		stakeDB:      stakeDB,
	}
}
...
// it into the BlockDataBasic and StakeInfoExtended and bitumdataapi types.
...
func (t *Collector) CollectBlockInfo(hash *chainhash.Hash) (*apitypes.BlockDataBasic,
	*bitumjson.FeeInfoBlock, *bitumjson.GetBlockHeaderVerboseResult,
	*apitypes.BlockExplorerExtraInfo, *wire.MsgBlock, error) {
	// Retrieve block from bitumd.
	msgBlock, err := t.bitumdChainSvr.GetBlock(hash)
	if err != nil {
	...
	coinSupply, err := t.bitumdChainSvr.GetCoinSupply()
	if err != nil {
		log.Error("GetCoinSupply failed: ", err)
	}
	nbSubsidy, err := t.bitumdChainSvr.GetBlockSubsidy(int64(msgBlock.Header.Height)+1, 5)
	if err != nil {
		log.Errorf("GetBlockSubsidy for %d failed: %v", msgBlock.Header.Height, err)
	}

	// Block header
	blockHeaderResults, err := t.bitumdChainSvr.GetBlockHeaderVerbose(hash)
	if err != nil {
		return nil, nil, nil, nil, nil, err
	}
	...
	sdiff := bitumutil.Amount(header.SBits).ToCoin()
...
func (t *Collector) CollectHash(hash *chainhash.Hash) (*BlockData, *wire.MsgBlock, error) {
	...
	// still running, or bitumd may be mad.
	t.mtx.Lock()
	defer t.mtx.Unlock()
	...
	numConn, err := t.bitumdChainSvr.GetConnectionCount()
	if err != nil {
	...
	chainInfo, err := t.bitumdChainSvr.GetBlockChainInfo()
	if err != nil {
	...
	blockdata := &BlockData{
		...
		FeeInfo:          *feeInfoBlock,
		CurrentStakeDiff: bitumjson.GetStakeDifficultyResult{CurrentStakeDifficulty: blockDataBasic.StakeDiff},
		EstStakeDiff:     bitumjson.EstimateStakeDiffResult{},
		PoolInfo:         blockDataBasic.PoolInfo,
		ExtraInfo:        *extra,
...
func (t *Collector) Collect() (*BlockData, *wire.MsgBlock, error) {
	...
	go func() {
		blockchainInfo, err := t.bitumdChainSvr.GetBlockChainInfo()
	...
	case <-time.After(time.Second * 10):
		log.Errorf("Timeout waiting for bitumd.")
	...
	// Stake difficulty
	stakeDiff, err := t.bitumdChainSvr.GetStakeDifficulty()
	if err != nil {
		return nil, nil, err
	}

	// estimatestakediff
	estStakeDiff, err := t.bitumdChainSvr.EstimateStakeDiff(nil)
	if err != nil {
