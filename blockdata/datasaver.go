...
func (s *BlockDataToSummaryStdOut) Store(data *BlockData, _ *wire.MsgBlock) error {
	...
	if data.PoolInfo != nil && data.PoolInfo.Value >= 0 {
		fmt.Printf("  Ticket pool:  %v (size), %.3f (avg. price), %.2f (total BITUM locked)\n",
...
// BlockTrigger wraps a simple function of builtin-typed hash and height that
// reports no error.
type BlockTrigger struct {
	Saver func(string, uint32)
}
