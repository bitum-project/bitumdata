...
var host = flag.String("host", "127.0.0.1:9209", "node RPC host:port")
var user = flag.String("user", "bitumd", "node RPC username")
var pass = flag.String("pass", "bananas", "node RPC password")
var cert = flag.String("cert", "bitumd.cert", "node RPC TLS certificate (when notls=false)")
...
func mainCore() int {
	...
	blocks := make(map[int64]*bitumutil.Block)
	...
	for i := int64(0); i < height+1; i++ {
		...
		blocks[i] = bitumutil.NewBlock(msgBlock)
		...
		blockSummaries[i] = apitypes.BlockDataBasic{
			...
			StakeDiff:  bitumutil.Amount(header.SBits).ToCoin(),
	...
	for i := range blockSummaries {
		blockSummaries[i].PoolInfo.Value = bitumutil.Amount(poolValues[i]).ToCoin()
