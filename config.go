// Copyright (c) 2016-2019 The Bitum developers
...
import (
	...
	"github.com/bitum-project/bitumdata/netparams"
	"github.com/bitum-project/bitumdata/version"
	"github.com/decred/slog"
	flags "github.com/jessevdk/go-flags"
)

const (
	defaultConfigFilename = "bitumdata.conf"
	defaultLogFilename    = "bitumdata.log"
...
var (
	defaultHomeDir           = bitumutil.AppDataDir("bitumdata", false)
	defaultConfigFile        = filepath.Join(defaultHomeDir, defaultConfigFilename)
	defaultLogDir            = filepath.Join(defaultHomeDir, defaultLogDirname)
	defaultDataDir           = filepath.Join(defaultHomeDir, defaultDataDirname)
	bitumdHomeDir              = bitumutil.AppDataDir("bitumd", false)
	defaultDaemonRPCCertFile = filepath.Join(bitumdHomeDir, "rpc.cert")
	...
	defaultDBFileName        = "bitumdata.sqlt.db"
	...
	defaultMainnetLink = "https://explorer.bitum.io/"
	defaultTestnetLink = "https://testnet.bitum.io/"
...
type config struct {
	...
	HomeDir      string `short:"A" long:"appdata" description:"Path to application home directory" env:"BITUMDATA_APPDATA_DIR"`
	ConfigFile   string `short:"C" long:"configfile" description:"Path to configuration file" env:"BITUMDATA_CONFIG_FILE"`
	DataDir      string `short:"b" long:"datadir" description:"Directory to store data" env:"BITUMDATA_DATA_DIR"`
	LogDir       string `long:"logdir" description:"Directory to log output." env:"BITUMDATA_LOG_DIR"`
	OutFolder    string `short:"f" long:"outfolder" description:"Folder for file outputs" env:"BITUMDATA_OUT_FOLDER"`
	ShowVersion  bool   `short:"V" long:"version" description:"Display version information and exit"`
	TestNet      bool   `long:"testnet" description:"Use the test network (default mainnet)" env:"BITUMDATA_USE_TESTNET"`
	SimNet       bool   `long:"simnet" description:"Use the simulation test network (default mainnet)" env:"BITUMDATA_USE_SIMNET"`
	DebugLevel   string `short:"d" long:"debuglevel" description:"Logging level {trace, debug, info, warn, error, critical}" env:"BITUMDATA_LOG_LEVEL"`
	Quiet        bool   `short:"q" long:"quiet" description:"Easy way to set debuglevel to error" env:"BITUMDATA_QUIET"`
	HTTPProfile  bool   `long:"httpprof" short:"p" description:"Start HTTP profiler." env:"BITUMDATA_ENABLE_HTTP_PROFILER"`
	HTTPProfPath string `long:"httpprofprefix" description:"URL path prefix for the HTTP profiler." env:"BITUMDATA_HTTP_PROFILER_PREFIX"`
	CPUProfile   string `long:"cpuprofile" description:"File for CPU profiling." env:"BITUMDATA_CPU_PROFILER_FILE"`
	UseGops      bool   `short:"g" long:"gops" description:"Run with gops diagnostics agent listening. See github.com/google/gops for more information." env:"BITUMDATA_USE_GOPS"`

	// API
	APIProto            string  `long:"apiproto" description:"Protocol for API (http or https)" env:"BITUMDATA_ENABLE_HTTPS"`
	APIListen           string  `long:"apilisten" description:"Listen address for API" env:"BITUMDATA_LISTEN_URL"`
	...
	UseRealIP           bool    `long:"userealip" description:"Use the RealIP middleware from the pressly/chi/middleware package to get the client's real IP from the X-Forwarded-For or X-Real-IP headers, in that order." env:"BITUMDATA_USE_REAL_IP"`
	CacheControlMaxAge  int     `long:"cachecontrol-maxage" description:"Set CacheControl in the HTTP response header to a value in seconds for clients to cache the response. This applies only to FileServer routes." env:"BITUMDATA_MAX_CACHE_AGE"`
	InsightReqRateLimit float64 `long:"insight-limit-rps" description:"Requests/second per client IP for the Insight API's rate limiter." env:"BITUMDATA_INSIGHT_RATE_LIMIT"`
	...
	AgendasDBFileName  string `long:"agendadbfile" description:"Agendas DB file name (default is agendas.db)." env:"BITUMDATA_AGENDAS_DB_FILE_NAME"`
	...
	FullMode       bool          `long:"pg" description:"Run in \"Full Mode\" mode,  enables postgresql support" env:"BITUMDATA_ENABLE_FULL_MODE"`
	PGDBName       string        `long:"pgdbname" description:"PostgreSQL DB name." env:"BITUMDATA_PG_DB_NAME"`
	PGUser         string        `long:"pguser" description:"PostgreSQL DB user." env:"BITUMDATA_POSTGRES_USER"`
	PGPass         string        `long:"pgpass" description:"PostgreSQL DB password." env:"BITUMDATA_POSTGRES_PASS"`
	PGHost         string        `long:"pghost" description:"PostgreSQL server host:port or UNIX socket (e.g. /run/postgresql)." env:"BITUMDATA_POSTGRES_HOST_URL"`
	...
	NoDevPrefetch    bool `long:"no-dev-prefetch" description:"Disable automatic dev fund balance query on new blocks. When true, the query will still be run on demand, but not automatically after new blocks are connected." env:"BITUMDATA_DISABLE_DEV_PREFETCH"`
	SyncAndQuit      bool `long:"sync-and-quit" description:"Sync to the best block and exit. Do not start the explorer or API." env:"BITUMDATA_ENABLE_SYNC_N_QUIT"`
	ImportSideChains bool `long:"import-side-chains" description:"(experimental) Enable startup import of side chains retrieved from bitumd via getchaintips." env:"BITUMDATA_IMPORT_SIDE_CHAINS"`
	...
	// EmailSubject string `long:"emailsubj" description:"Email subject. (default \"bitumdataapi transaction notification\")"`

	// RPC client options
	BitumdUser         string `long:"bitumduser" description:"Daemon RPC user name" env:"BITUMDATA_BITUMD_USER"`
	BitumdPass         string `long:"bitumdpass" description:"Daemon RPC password" env:"BITUMDATA_BITUMD_PASS"`
	...
	DisabledExchanges string `long:"disable-exchange" description:"Exchanges to disable. See /exchanges/exchanges.go for available exchanges. Use a comma to separate multiple exchanges" env:"BITUMDATA_DISABLE_EXCHANGES"`
	ExchangeCurrency  string `long:"exchange-currency" description:"The default bitcoin price index. A 3-letter currency code" env:"BITUMDATA_EXCHANGE_INDEX"`
	RateMaster        string `long:"ratemaster" description:"The address of a BITUMRates instance. Exchange monitoring will get all data from a BITUMRates subscription." env:"BITUMDATA_RATE_MASTER"`
	RateCertificate   string `long:"ratecert" description:"File containing BITUMRates TLS certificate file." env:"BITUMDATA_RATE_MASTER"`

	// Links
	MainnetLink string `long:"mainnet-link" description:"When bitumdata is on testnet, this address will be used to direct a user to a bitumdata on mainnet when appropriate." env:"BITUMDATA_MAINNET_LINK"`
	TestnetLink string `long:"testnet-link" description:"When bitumdata is on mainnet, this address will be used to direct a user to a bitumdata on testnet when appropriate." env:"BITUMDATA_TESTNET_LINK"`
}
...
var (
	defaultConfig = config{
		...
		BitumdCert:            defaultDaemonRPCCertFile,
...
func loadConfig() (*config, error) {
	...
	// them.
	cfg.BitumdServ, err = normalizeNetworkAddress(cfg.BitumdServ, defaultHost, activeNet.JSONRPCClientPort)
	if err != nil {
		return loadConfigError(err)
	}
	...
	cfg.BitumdCert = cleanAndExpandPath(cfg.BitumdCert)
...
func netName(chainParams *netparams.Params) string {
	...
	// TestNet2 was removed entirely for bitumd 1.3.0. Compile check!
	switch chainParams.Net {
	case wire.TestNet, wire.MainNet, wire.SimNet:
	default:
