...
func TestMain(m *testing.M) {
	...
	tempConfigFile, _ = ioutil.TempFile("", "bitumdata_test_file.cfg")
	defer os.Remove(tempConfigFile.Name())
	os.Setenv("BITUMDATA_CONFIG_FILE", tempConfigFile.Name())

	// Make an empty folder for appdata tests.
	tempAppDataDir, _ = ioutil.TempDir("", "bitumdata_test_appdata")
	...
	retCode := m.Run()
	os.Unsetenv("BITUMDATA_CONFIG_FILE")

	os.Exit(retCode)
}

// disableConfigFileEnv checks if the BITUMDATA_CONFIG_FILE environment variable
// is set, unsets it, and returns a function that will return
// BITUMDATA_CONFIG_FILE to its state before calling disableConfigFileEnv.
func disableConfigFileEnv() func() {
	loc, wasSet := os.LookupEnv("BITUMDATA_CONFIG_FILE")
	if wasSet {
		os.Unsetenv("BITUMDATA_CONFIG_FILE")
		return func() { os.Setenv("BITUMDATA_CONFIG_FILE", loc) }
	}
	return func() {}
}
...
func TestLoadCustomConfigPresent(t *testing.T) {
	...
	_, err := loadConfig()
	if err != nil {
		t.Fatalf("Failed to load bitumdata config: %v", err)
	}
}
...
func TestLoadDefaultConfigMissing(t *testing.T) {
	...
	os.Setenv("BITUMDATA_APPDATA_DIR", tempAppDataDir)
	defer os.Unsetenv("BITUMDATA_APPDATA_DIR")
...
func TestLoadCustomConfigMissing(t *testing.T) {
	...
	os.Setenv("BITUMDATA_CONFIG_FILE", goneFile.Name())

	// Attempt to load using the non-existent non-default config file, which
	// should return an error.
	_, err := loadConfig()
	if err == nil {
		t.Errorf("Loaded bitumdata config, but the explicitly set config file"+
...
func TestDefaultConfigAPIListenWithEnv(t *testing.T) {
	...
	os.Setenv("BITUMDATA_LISTEN_URL", customListenPath)
...
func TestDefaultConfigAppDataDir(t *testing.T) {
	expected := bitumutil.AppDataDir("bitumdata", false)
	cfg, err := loadConfig()
	if err != nil {
		t.Fatalf("Failed to load bitumdata config: %v", err)
	}
...
func TestDefaultConfigHomeDirWithEnvAndFlag(t *testing.T) {
	...
	os.Setenv("BITUMDATA_APPDATA_DIR", cliOverride)
	defer os.Unsetenv("BITUMDATA_APPDATA_DIR")
...
func TestDefaultConfigTestNetWithEnv(t *testing.T) {
	os.Setenv("BITUMDATA_USE_TESTNET", "true")
	defer os.Unsetenv("BITUMDATA_USE_TESTNET")
...
func TestDefaultConfigTestNetWithEnvAndBadValue(t *testing.T) {
	os.Setenv("BITUMDATA_USE_TESTNET", "no")
	defer os.Unsetenv("BITUMDATA_USE_TESTNET")

	_, err := loadConfig()
	if err == nil {
		t.Errorf("Invalid boolean value for BITUMDATA_USE_TESTNET did not cause an error.")
	}
}
