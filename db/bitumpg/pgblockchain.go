...
import (
	...
	"github.com/bitum-project/bitumdata/db/bitumpg/internal"
...
type BlockGetter interface {
	...
	GetBlockChainInfo() (*bitumjson.GetBlockChainInfoResult, error)
}
...
// DB. Side chains known to bitumd are listed via the getchaintips RPC. Each block
...
// NewChainDB constructs a ChainDB for the given connection and Bitum network
...
// connection and Bitum network parameters. By default, duplicate row checks on
...
func NewChainDBWithCancel(ctx context.Context, dbi *DBInfo, params *chaincfg.Params,
	...
	if !doLegacyUpgrade {
		...
		// table to match the blocks table, allowing bitumdata to start.
...
func (pgb *ChainDB) AddressHistory(address string, N, offset int64,
	...
	log.Infof("%s: %d spent totalling %f BITUM, %d unspent totalling %f BITUM",
		address, balance.NumSpent, bitumutil.Amount(balance.TotalSpent).ToCoin(),
		balance.NumUnspent, bitumutil.Amount(balance.TotalUnspent).ToCoin())
...
SPENDING_TX_DUPLICATE_CHECK:
	for _, f := range addressUTXOs.PrevOuts {
		...
		// outpoint because vin:i valuein is not reliable from bitumd.
		...
		if txnType == dbtypes.AddrTxnAll || txnType == dbtypes.AddrTxnDebit {
			addrTx := &dbtypes.AddressTx{
				...
				SentTotal:      bitumutil.Amount(valuein).ToCoin(),
...
func (pgb *ChainDB) FillAddressTransactions(addrInfo *dbtypes.AddressInfo) error {
	...
	for i, txn := range addrInfo.Transactions {
		...
		txn.Total = bitumutil.Amount(dbTx.Sent).ToCoin()
...
func (pgb *ChainDB) AddressTotals(address string) (*apitypes.AddressTotals, error) {
	...
	return &apitypes.AddressTotals{
		...
		CoinsSpent:   bitumutil.Amount(ab.TotalSpent).ToCoin(),
		CoinsUnspent: bitumutil.Amount(ab.TotalUnspent).ToCoin(),
	}, nil
}
...
func MakeCsvAddressRows(rows []*dbtypes.AddressRow) [][]string {
	...
	for _, r := range rows {
		...
		csvRows = append(csvRows, []string{
			...
			strValidMainchain,
			strconv.FormatFloat(bitumutil.Amount(r.Value).ToCoin(), 'f', -1, 64),
...
func (pgb *ChainDB) UpdateChainState(blockChainInfo *bitumjson.GetBlockChainInfoResult) {
	...
	if blockChainInfo == nil {
		log.Errorf("bitumjson.GetBlockChainInfoResult data passed is empty")
		return
	}
