...
func InsertTickets(db *sql.DB, dbTxns []*dbtypes.Tx, txDbIDs []uint64, checked, updateExistingRecords bool) ([]uint64, []*dbtypes.Tx, error) {
	...
	for i, tx := range ticketTx {
		...
		price := bitumutil.Amount(tx.Vouts[0].Value).ToCoin()
		fee := bitumutil.Amount(tx.Fees).ToCoin()
		isSplit := tx.NumVin > 1
...
func InsertVotes(db *sql.DB, dbTxns []*dbtypes.Tx, _ /*txDbIDs*/ []uint64, fTx *TicketTxnIDGetter,
	...
	for i, tx := range voteTxs {
		...
		voteReward := bitumutil.Amount(msgTx.TxIn[0].ValueIn).ToCoin()
		stakeSubmissionAmount := bitumutil.Amount(msgTx.TxIn[1].ValueIn).ToCoin()
...
func retrieveTicketsByDate(ctx context.Context, db *sql.DB, maturityBlock int64, groupBy string) (*dbtypes.PoolTicketsData, error) {
	...
	for rows.Next() {
		...
		tickets.Price = append(tickets.Price, bitumutil.Amount(price/total).ToCoin())
	}
...
func RetrieveAddressUTXOs(ctx context.Context, db *sql.DB, address string, currentBlockHeight int64) ([]*apitypes.AddressTxnOutput, error) {
	...
	for rows.Next() {
		...
		txnOutput.Amount = bitumutil.Amount(atoms).ToCoin()
...
func retrieveAddressIoCsv(ctx context.Context, db *sql.DB, address string) (csvRows [][]string, err error) {
	...
	for dbRows.Next() {
		...
		csvRows = append(csvRows, []string{
			...
			strValidMainchain,
			strconv.FormatFloat(bitumutil.Amount(value).ToCoin(), 'f', -1, 64),
...
// (hash:index).
// Update Vin due to BITUMD AMOUNTIN - START - DO NOT MERGE CHANGES IF BITUMD FIXED
...
func retrieveTxHistoryByAmountFlow(ctx context.Context, db *sql.DB, addr, timeInterval string) (*dbtypes.ChartsData, error) {
	...
	for rows.Next() {
		...
		items.Time = append(items.Time, dbtypes.NewTimeDef(blockTime))
		items.Received = append(items.Received, bitumutil.Amount(received).ToCoin())
		items.Sent = append(items.Sent, bitumutil.Amount(sent).ToCoin())
		...
		items.Net = append(items.Net, bitumutil.Amount(received-sent).ToCoin())
	}
	return items, nil
}
...
// used by the BITUM to Insight transaction converter.
