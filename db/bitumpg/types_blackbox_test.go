...
func openDB() (func() error, error) {
	dbi := bitumpg.DBInfo{
		Host:   "localhost",
		Port:   "5432",
		User:   "bitumdata", // postgres for admin operations
		Pass:   "",
		DBName: "bitumdata_mainnet_test",
	}
	var err error
	// Connect to the PostgreSQL daemon and return the *sql.DB.
	db, err = bitumpg.Connect(dbi.Host, dbi.Port, dbi.User, dbi.Pass, dbi.DBName)
	if err != nil {
		return nil, err
	}
	if err = bitumpg.DropTestingTable(db); err != nil {
		return nil, err
	}
	if err = bitumpg.CreateTable(db, "testing"); err != nil {
		return nil, err
	}
...
func TestTimeRoundTripCorrectTimeDef(t *testing.T) {
	...
	if err := bitumpg.ClearTestingTable(db); err != nil {
