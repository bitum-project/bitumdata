...
func (db *WiredDB) ReportHeights() error {
	...
	if stakeInfoHeight != summaryHeight {
		err = fmt.Errorf("SQLite database (bitumdata.sqlt.db) is corrupted")
	}
...
func makeBlockTransactions(blockVerbose *bitumjson.GetBlockVerboseResult) *apitypes.BlockTransactions {
...
func (db *WiredDB) GetAllTxIn(txid *chainhash.Hash) []*apitypes.TxIn {
	...
	for i := range allTxIn {
		txIn := &apitypes.TxIn{
			...
			ValueIn:         bitumutil.Amount(allTxIn0[i].ValueIn).ToCoin(),
...
func (db *WiredDB) GetAllTxOut(txid *chainhash.Hash) []*apitypes.TxOut {
	...
	for i := range txouts {
		// bitumjson.Vout and apitypes.TxOut are the same except for N.
		spk := &tx.Vout[i].ScriptPubKey
		// If the script type is not recognized by apitypes, the ScriptClass
		// types may need to be updated to match bitumd.
		if spk.Type != "invalid" && !apitypes.IsValidScriptClass(spk.Type) {
			log.Warnf(`The ScriptPubKey's type "%s" is not known to bitumdata! ` +
				`Update apitypes or debug bitumd.`)
		}
...
// on the stake version with which bitumdata is compiled with (chaincfg.Params),
...
func (db *WiredDB) GetSummaryByHash(hash string, withTxTotals bool) *apitypes.BlockDataBasic {
	...
	if withTxTotals {
		...
		var totalFees, totalOut bitumutil.Amount
...
func (db *WiredDB) GetAddressTransactionsWithSkip(addr string, count, skip int) *apitypes.Address {
	address, err := bitumutil.DecodeAddress(addr)
	if err != nil {
...
func (db *WiredDB) GetAddressTransactionsRawWithSkip(addr string, count int, skip int) []*apitypes.AddressTxRaw {
	...
	for i := range txs {
		...
		tx.Vin = make([]bitumjson.VinPrevOut, len(txs[i].Vin))
...
func sumOutsTxRawResult(txs []bitumjson.TxRawResult) (sum float64) {
	for _, tx := range txs {
...
func makeExplorerBlockBasic(data *bitumjson.GetBlockVerboseResult, params *chaincfg.Params) *exptypes.BlockBasic {
...
func makeExplorerTxBasic(data bitumjson.TxRawResult, msgTx *wire.MsgTx, params *chaincfg.Params) *exptypes.TxBasic {
...
func makeExplorerAddressTx(data *bitumjson.SearchRawTransactionsResult, address string) *dbtypes.AddressTx {
...
func (db *WiredDB) GetExplorerBlock(hash string) *exptypes.BlockInfo {
	...
	for _, tx := range data.RawSTx {
		...
		case stake.TxTypeSSGen:
			...
			// well.
			if stx.Fee > 0 {
				log.Tracef("Vote with fee! %v, %v BITUM", stx.Fee, stx.Fees)
			}
	...
	getTotalFee := func(txs []*exptypes.TrimmedTxInfo) (total bitumutil.Amount) {
		for _, tx := range txs {
	...
	getTotalSent := func(txs []*exptypes.TrimmedTxInfo) (total bitumutil.Amount) {
		for _, tx := range txs {
			amt, err := bitumutil.NewAmount(tx.Total)
			if err != nil {
				continue
			}
			total += amt
...
func trimmedTxInfoFromMsgTx(txraw bitumjson.TxRawResult, msgTx *wire.MsgTx, params *chaincfg.Params) *exptypes.TrimmedTxInfo {
...
func (db *WiredDB) GetExplorerTx(txid string) *exptypes.TxInfo {
	...
	for i, vin := range txraw.Vin {
		...
		valueIn, _ := bitumutil.NewAmount(vin.AmountIn)
...
func (db *WiredDB) GetExplorerAddress(address string, count, offset int64) (*dbtypes.AddressInfo, txhelpers.AddressType, txhelpers.AddressError) {
	...
	var totalreceived, totalsent bitumutil.Amount
		...
		for _, y := range tx.Vout {
			if len(y.ScriptPubKey.Addresses) != 0 {
				if address == y.ScriptPubKey.Addresses[0] {
					t, _ := bitumutil.NewAmount(y.Value)
					if t > 0 {
						totalreceived += t
					}
					numReceiving++
		...
		for _, u := range tx.Vin {
			if u.PrevOut != nil && len(u.PrevOut.Addresses) != 0 {
				if address == u.PrevOut.Addresses[0] {
					t, _ := bitumutil.NewAmount(*u.AmountIn)
					if t > 0 {
						totalsent += t
					}
					numSpending++
...
func (db *WiredDB) GetMempool() []exptypes.MempoolTx {
	mempooltxs, err := db.client.GetRawMempoolVerbose(bitumjson.GRMAll)
	if err != nil {
