...
import (
	...
	"github.com/bitum-project/bitumdata/db/dbtypes/internal"
...
const (
	...
	// be effected. https://docs.bitum.io/glossary/#rule-change-interval-rci.
...
// Tx models a Bitum transaction. It is stored in a Block.
type Tx struct {
	//blockDbID  int64
...
type AddressInfo struct {
	...
	AmountReceived  bitumutil.Amount
	AmountSent      bitumutil.Amount
	AmountUnspent   bitumutil.Amount
...
func ReduceAddressHistory(addrHist []*AddressRow) (*AddressInfo, float64, float64) {
	...
	for _, addrOut := range addrHist {
		...
		coin := bitumutil.Amount(addrOut.Value).ToCoin()
	...
	ai := &AddressInfo{
		...
		AmountReceived:  bitumutil.Amount(received),
		AmountSent:      bitumutil.Amount(sent),
		AmountUnspent:   bitumutil.Amount(received - sent),
	}
