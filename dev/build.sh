#!/bin/bash
# Usage: build.sh [bitumdata_root] [destination_folder]
#
...
#   root folder. If not running from a git repository, the bitumdata_root folder
#   must be specified.
#
#   Specify destination_folder to install the bitumdata executable and the static
...
if [[ -z "$ROOT" ]]; then
    echo "Not in git repository. You must specify the bitumdata root folder as the first argument!"
    exit 1
fi
...
echo "Building the bitumdata binary..."
GO111MODULE=on go build -v -o bitumdata
...
if [[ -n "$DEST" ]]; then
    sudo install -m 754 -o bitumdata -g bitum ./bitumdata ${DEST}/
