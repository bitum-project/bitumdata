#!/bin/sh
#
# Script to setup parallel bitumd nodes with separate wallets.
# Useful for testing reorgs by disconnecting nodes, mining individually, then
# reconnecting them.
#
#             alpha  <------>  beta
#    listen   19200           19200
# rpclisten   19201 <.     .> 19201
#           w-alpha  |     | w-beta
# rpclisten   19202           19202
#
...
cat > "${NODES_ROOT}/bitumd.conf" <<EOF
rpcuser=${RPCUSER}
rpcpass=${RPCPASS}
simnet=1
logdir=./log
...
cp ${NODES_ROOT}/bitumd.conf ${NODES_ROOT}/alpha
cat >> ${NODES_ROOT}/alpha/bitumd.conf <<EOF
listen=127.0.0.1:19200
rpclisten=127.0.0.1:19201
miningaddr=${WALLET01_MININGADDR}
EOF

cp ${NODES_ROOT}/bitumctl.conf ${NODES_ROOT}/alpha
cat >> ${NODES_ROOT}/alpha/bitumctl.conf <<EOF
rpcserver=127.0.0.1:19201
EOF

cp ${NODES_ROOT}/bitumctl.conf ${NODES_ROOT}/w-alpha
cat >> ${NODES_ROOT}/w-alpha/bitumctl.conf <<EOF
walletrpcserver=127.0.0.1:19202
EOF


cp ${NODES_ROOT}/bitumd.conf ${NODES_ROOT}/beta
cat >> ${NODES_ROOT}/beta/bitumd.conf <<EOF
listen=127.0.0.1:19200
rpclisten=127.0.0.1:19201
miningaddr=${WALLET02_MININGADDR}
EOF

cp ${NODES_ROOT}/bitumctl.conf ${NODES_ROOT}/beta
cat >> ${NODES_ROOT}/beta/bitumctl.conf <<EOF
rpcserver=127.0.0.1:19201
EOF

cp ${NODES_ROOT}/bitumctl.conf ${NODES_ROOT}/w-beta
cat >> ${NODES_ROOT}/w-beta/bitumctl.conf <<EOF
walletrpcserver=127.0.0.1:19202
EOF
...
#!/bin/sh
bitumctl -C ./bitumctl.conf --wallet -c ./data/rpc.cert \$*
EOF
...
tmux send-keys "bitumd -C ./bitumd.conf" C-m
...
tmux send-keys "bitumd -C ./bitumd.conf --connect 127.0.0.1:19200" C-m
tmux select-pane -t 1
tmux send-keys "cd beta" C-m
#sleep 3
#tmux send-keys "./ctl generate 16" C-m
...
tmux send-keys "bitumwallet -C ../wallet.conf --rpcconnect 127.0.0.1:19201 \
