...
const (
	...
	defaultBITUMRatesPort = "7778"

	aggregatedOrderbookKey = "aggregated"
	orderbookKey           = "depth"
)

var grpcClient bitumrates.BITUMRatesClient
...
type ExchangeBot struct {
	mtx             sync.RWMutex
	BitumBtcExchanges map[string]Exchange
	IndexExchanges  map[string]Exchange
	Exchanges       map[string]Exchange
	versionedCharts map[string]*versionedChart
	chartVersions   map[string]int
	// BtcIndex is the (typically fiat) currency to which the BITUM price should be
	...
	// exchanges or no up-to-date Bitum exchanges. IsFailed is a getter for failed.
	failed bool
}

// ExchangeBotState is the current known state of all exchanges, in a certain
// base currency, and a volume-averaged price and total volume in BITUM.
type ExchangeBotState struct {
	BtcIndex    string                    `json:"btc_index"`
	BtcPrice    float64                   `json:"btc_fiat_price"`
	Price       float64                   `json:"price"`
	Volume      float64                   `json:"volume"`
	BitumBtc      map[string]*ExchangeState `json:"bitum_btc_exchanges"`
...
func (state ExchangeBotState) copy() *ExchangeBotState {
	state.BitumBtc = copyStates(state.BitumBtc)
...
func (state *ExchangeBotState) VolumeOrderedExchanges() []*tokenedExchange {
	xcList := make([]*tokenedExchange, 0, len(state.BitumBtc))
	for token, state := range state.BitumBtc {
...
func NewExchangeBot(config *ExchangeBotConfig) (*ExchangeBot, error) {
	...
	bot := &ExchangeBot{
		BitumBtcExchanges: make(map[string]Exchange),
		...
		currentState: ExchangeBotState{
			...
			Price:       0,
			Volume:      0,
			BitumBtc:      make(map[string]*ExchangeState),
	...
	for token, constructor := range BitumExchanges {
		buildExchange(token, constructor, bot.BitumBtcExchanges)
	}

	if len(bot.BitumBtcExchanges) == 0 {
		return nil, fmt.Errorf("No BITUM-BTC exchanges were intitialized")
	}
...
func (bot *ExchangeBot) Start(ctx context.Context, wg *sync.WaitGroup) {
	...
	if config.MasterBot != "" {
		...
		if err != nil {
			...
			// Start a loop to listen for updates from the bitumrates server.
			go func() {
				for {
					...
					if err != nil {
						...
						log.Errorf("BITUMRates error. Attempting to reconnect in 10 seconds: %v", err)
						...
						for {
							...
							if err == nil {
								...
								log.Errorf("Failed to reconnect to BITUMRates. Attempting to reconnect in %s: %v", delayString, err)
							}
						}
						log.Infof("BITUMRates connection re-established.")
						reconnectionAttempt = 0
						continue
					}
					// Send the update through the Exchange so that appropriate attributes
					// are set.
					if IsBitumExchange(update.Token) {
...
// Attempt BITUMRates connection after delay.
func (bot *ExchangeBot) connectMasterBot(ctx context.Context, delay time.Duration) (bitumrates.BITUMRates_SubscribeExchangesClient, error) {
	if bot.masterConnection != nil {
		bot.masterConnection.Close()
	}
	if delay > 0 {
	...
	grpcClient := bitumrates.NewBITUMRatesClient(conn)
	stream, err := grpcClient.SubscribeExchanges(ctx, &bitumrates.ExchangeSubscription{
...
func (bot *ExchangeBot) ConvertedState(code string) (*ExchangeBotState, error) {
	...
	bitumPrice, volume := bot.processState(bot.currentState.BitumBtc, true)
	btcPrice, _ := bot.processState(fiatIndices, false)
	if bitumPrice == 0 || btcPrice == 0 {
		bot.failed = true
	...
	state := ExchangeBotState{
		...
		Price:       bitumPrice * btcPrice,
		BitumBtc:      bot.currentState.BitumBtc,
...
// updateExchange processes an update from a Bitum-BTC Exchange.
func (bot *ExchangeBot) updateExchange(update *ExchangeUpdate) error {
	...
	bot.currentState.BitumBtc[update.Token] = update.State
...
func (bot *ExchangeBot) updateState() error {
	...
	if bitumPrice == 0 || btcPrice == 0 {
		...
		bot.failed = false
		bot.currentState.Price = bitumPrice * btcPrice
...
// sources or no Bitum Exchanges are up-to-date. Individual exchanges can
// be outdated/failed without IsFailed being false, as long as there is at least
// one Bitcoin index and one Bitum exchange.
...
// Conversion is a representation of some amount of BITUM in another index.
...
func (bot *ExchangeBot) Conversion(bitumVal float64) *Conversion {
	if bot == nil {
		return nil
	}
	xcState := bot.State()
...
func (bot *ExchangeBot) QuickSticks(token string, rawBin string) ([]byte, error) {
	...
	bot.mtx.Lock()
	defer bot.mtx.Unlock()
	state, found := bot.currentState.BitumBtc[token]
	if !found {
		return nil, fmt.Errorf("Failed to find BITUM exchange state for %s", token)
	}
...
func (bot *ExchangeBot) aggOrderbook() *aggregateOrderbook {
	...
	tokens := []string{}
	for token, xcState := range state.BitumBtc {
		if !xcState.HasDepth() {
			continue
		}
...
func (bot *ExchangeBot) QuickDepth(token string) (chart []byte, err error) {
	...
	if token == aggregatedOrderbookKey {
		...
		bot.mtx.Lock()
		defer bot.mtx.Unlock()
		xcState, found := bot.currentState.BitumBtc[token]
		if !found {
			return nil, fmt.Errorf("Failed to find BITUM exchange state for %s", token)
		}
