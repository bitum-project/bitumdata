...
import (
	...
	"github.com/bitum-project/bitumdata/explorer/types"
	"github.com/bitum-project/bitumdata/gov/agendas"
	pitypes "github.com/bitum-project/bitumdata/gov/politeia/types"
...
type explorerDataSourceLite interface {
	...
	DecodeRawTransaction(txhex string) (*bitumjson.TxRawResult, error)
	SendRawTransaction(txhex string) (string, error)
	GetHeight() (int64, error)
	GetChainParams() *chaincfg.Params
	GetMempool() []types.MempoolTx
	TxHeight(txid *chainhash.Hash) (height int64)
	BlockSubsidy(height int64, voters uint16) *bitumjson.GetBlockSubsidyResult
...
type links struct {
	...
	BtcAddress      string
	DownloadLink    string
	// Testnet and below are set via bitumdata config.
	Testnet       string
	Mainnet       string
...
var explorerLinks = &links{
	CoinbaseComment: "https://github.com/bitum-project/bitumd/blob/2a18beb4d56fe59d614a7309308d84891a0cba96/chaincfg/genesis.go#L17-L53",
	POSExplanation:  "https://docs.bitum.io/faq/proof-of-stake/general/#9-what-is-proof-of-stake-voting",
	APIDocs:         "https://github.com/bitum-project/bitumdata#apis",
	InsightAPIDocs:  "https://github.com/bitum-project/bitumdata/blob/master/api/Insight_API_documentation.md",
	Github:          "https://github.com/bitum-project/bitumdata",
	License:         "https://github.com/bitum-project/bitumdata/blob/master/LICENSE",
	NetParams:       "https://github.com/bitum-project/bitumd/blob/master/chaincfg/params.go",
	BtcAddress:      "https://live.blockcypher.com/btc/address/",
	DownloadLink:    "https://bitum.io/downloads/",
}
...
type pageData struct {
	...
	BlockchainInfo *bitumjson.GetBlockChainInfoResult
...
func (exp *explorerUI) Store(blockData *blockdata.BlockData, msgBlock *wire.MsgBlock) error {
	...
	// If BlockData contains non-nil PoolInfo, compute actual percentage of BITUM
	// supply staked.
	stakePerc := 45.0
	if blockData.PoolInfo != nil {
		stakePerc = blockData.PoolInfo.Value / bitumutil.Amount(blockData.ExtraInfo.CoinSupply).ToCoin()
	}
	// Simulate the annual staking rate
	ASR, _ := exp.simulateASR(1000, false, stakePerc,
		bitumutil.Amount(blockData.ExtraInfo.CoinSupply).ToCoin(),
	...
	posSubsPerVote := bitumutil.Amount(blockData.ExtraInfo.NextBlockSubsidy.PoS).ToCoin() /
...
// starting amount of BITUM and calculation parameters.  Generate a TEXT table of
// the simulation results that can optionally be used for future expansion of
// bitumdata functionality.
func (exp *explorerUI) simulateASR(StartingBITUMBalance float64, IntegerTicketQty bool,
	...
	StakeRewardAtBlock := func(blocknum float64) float64 {
		...
		return bitumutil.Amount(Subsidy.PoS).ToCoin()
		...
		// return float64(RewardProportionPerVote) * bitumutil.Amount(exp.ChainParams.BaseSubsidy).ToCoin() *
	...
	MaxCoinSupplyAtBlock := func(blocknum float64) float64 {
		// 4th order poly best fit curve to Bitum mainnet emissions plot.
	...
	BITUMBalance := StartingBITUMBalance

	ReturnTable += fmt.Sprintf("\n\nBLOCKNUM        BITUM  TICKETS TKT_PRICE TKT_REWRD  ACTION\n")
	ReturnTable += fmt.Sprintf("%8d  %9.2f %8.1f %9.2f %9.2f    INIT\n",
		int64(simblock), BITUMBalance, TicketsPurchased,
	...
	for simblock < (BlocksPerYear + CurrentBlockNum) {
		...
		if IntegerTicketQty {
			...
			TicketsPurchased = math.Floor(BITUMBalance / TicketPrice)
		} else {
			// Use this to simulate ALL funds used to buy tickets - even fractional tickets
			// which is actually not possible
			TicketsPurchased = (BITUMBalance / TicketPrice)
		}

		BITUMBalance -= (TicketPrice * TicketsPurchased)
		...
		// Simulate reward
		BITUMBalance += (StakeRewardAtBlock(simblock) * TicketsPurchased)
		TicketsPurchased = 0
	...
	SimulationReward := ((BITUMBalance - StartingBITUMBalance) / StartingBITUMBalance) * 100
...
// mempoolTime is the TimeDef that the transaction was received in BITUMData, or
