...
// netName returns the name used when referring to a bitum network.
...
func (exp *explorerUI) Home(w http.ResponseWriter, r *http.Request) {
	...
	if xcBot != nil {
		conversions = &homeConversions{
			...
			CoinSupply:      xcBot.Conversion(bitumutil.Amount(homeInfo.CoinSupply).ToCoin()),
			PowSplit:        xcBot.Conversion(bitumutil.Amount(homeInfo.NBlockSubsidy.PoW).ToCoin()),
			TreasurySplit:   xcBot.Conversion(bitumutil.Amount(homeInfo.NBlockSubsidy.Dev).ToCoin()),
			TreasuryBalance: xcBot.Conversion(bitumutil.Amount(homeInfo.DevFund).ToCoin()),
		}
	}
...
func (exp *explorerUI) TxPage(w http.ResponseWriter, r *http.Request) {
	...
	// If bitumd has no information about the transaction, pull the transaction
	...
	if tx == nil {
		...
		dbTx0 := dbTxs[0]
		fees := bitumutil.Amount(dbTx0.Fees)
		tx = &types.TxInfo{
			TxBasic: &types.TxBasic{
				TxID:          hash,
				FormattedSize: humanize.Bytes(uint64(dbTx0.Size)),
				Total:         bitumutil.Amount(dbTx0.Sent).ToCoin(),
				Fee:           fees,
				FeeRate:       bitumutil.Amount((1000 * int64(fees)) / int64(dbTx0.Size)),
		...
		for iv := range vouts {
			...
			amount := bitumutil.Amount(int64(vouts[iv].Value)).ToCoin()
		...
		for iv := range vins {
			...
			amount := bitumutil.Amount(vins[iv].ValueIn).ToCoin()
	...
	if tx.Type == types.CoinbaseTypeStr {
		...
		if data == nil {
			...
			feeAmt, _ := bitumutil.NewAmount(data.MiningFee)
...
func (exp *explorerUI) AddressPage(w http.ResponseWriter, r *http.Request) {
	...
	conversion := exp.xcBot.Conversion(bitumutil.Amount(addrData.Balance.TotalUnspent).ToCoin())
...
func (exp *explorerUI) commonData(r *http.Request) *CommonPageData {
	...
	if err != nil && err != http.ErrNoCookie {
		log.Errorf("Cookie bitumdataDarkBG retrieval error: %v", err)
	}
