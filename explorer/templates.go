...
import (
	...
	"github.com/bitum-project/bitumd/bitumec"
...
func makeTemplateFuncMap(params *chaincfg.Params) template.FuncMap {
	...
	return template.FuncMap{
		...
		"amountAsDecimalParts": func(v int64, useCommas bool) []string {
			return float64Formatting(bitumutil.Amount(v).ToCoin(), 8, useCommas)
		},
		"toFloat64Amount": func(intAmount int64) float64 {
			return bitumutil.Amount(intAmount).ToCoin()
		},
		...
		"PKAddr2PKHAddr": func(address string) (p2pkh string) {
			...
			addrPKH, err := bitumutil.NewAddressPubKeyHash(addrHash[:], addr.Net(), bitumec.STEcdsaSecp256k1)
			if err != nil {
				log.Errorf(err.Error())
				return ""
			}
