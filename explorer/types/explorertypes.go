...
type TxBasic struct {
	...
	Total         float64
	Fee           bitumutil.Amount
	FeeRate       bitumutil.Amount
	VoteInfo      *VoteInfo
	Coinbase      bool
}
...
type TrimmedBlockInfo struct {
	...
	Total        float64
	Fees         float64
	Subsidy      *bitumjson.GetBlockSubsidyResult
	Votes        []*TrimmedTxInfo
...
type HomeInfo struct {
	...
	TotalLockedBITUM        float64               `json:"total_locked_bitum"`
...
// BlockSubsidy is an implementation of bitumjson.GetBlockSubsidyResult
...
func (mpi *MempoolInfo) Trim() *TrimmedMempoolInfo {
	...
	getTotalFee := func(txs []*TrimmedTxInfo) bitumutil.Amount {
		var sum bitumutil.Amount
		for _, tx := range txs {
			sum += tx.TxBasic.Fee
		}
		return sum
...
func TrimMempoolTx(txs []MempoolTx) (trimmedTxs []*TrimmedTxInfo) {
	for _, tx := range txs {
		fee, _ := bitumutil.NewAmount(tx.Fees) // non-nil error returns 0 fee
		var feeRate bitumutil.Amount
		if tx.Size > 0 {
			feeRate = fee / bitumutil.Amount(int64(tx.Size))
		}
		txBasic := &TxBasic{
			TxID:          tx.TxID,
