...
// AgendaTagged has the same fields as bitumjson.Agenda plus the VoteVersion
...
type AgendaTagged struct {
	...
	Choices        []bitumjson.Choice         `json:"choices"`
...
type DeploymentSource interface {
	GetVoteInfo(version uint32) (*bitumjson.GetVoteInfoResult, error)
}
...
// bitumjson.GetVoteInfoResult and chaincfg.ConsensusDeployment hold almost similar
// data contents but chaincfg.Vote does not contain the important vote status
// field that is found in bitumjson.Agenda.
