...
func (*testClient) GetVoteInfo(version uint32) (*bitumjson.GetVoteInfoResult, error) {
	if version != 5 {
		return &bitumjson.GetVoteInfoResult{}, fmt.Errorf(voteVersionErrMsg, version)
	}
	resp := &bitumjson.GetVoteInfoResult{
		...
		Agendas: []bitumjson.Agenda{
			{
				ID:             "TestAgenda0001",
				Description:    "This agenda just shows bitumjson.GetVoteInfoResult payload format",
				Mask:           6,
