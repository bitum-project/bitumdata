...
type VoteDataSource interface {
	GetStakeVersionInfo(int32) (*bitumjson.GetStakeVersionInfoResult, error)
	GetVoteInfo(uint32) (*bitumjson.GetVoteInfoResult, error)
	GetStakeVersions(string, int32) (*bitumjson.GetStakeVersionsResult, error)
}

// bitumd does not supply vote counts for completed votes, so the tracker will
...
type VoteTracker struct {
	...
	blockVersion   int32
	stakeVersion   uint32
	stakeInfo      *bitumjson.GetStakeVersionInfoResult
	voteInfo       *bitumjson.GetVoteInfoResult
	summary        *VoteSummary
	ringIndex      int
	ringHeight     int64
	blockRing      []int32
...
// Since version could technically be updated without turning off bitumdata,
...
func (tracker *VoteTracker) refreshRCI() (*bitumjson.GetVoteInfoResult, error) {
	oldVersion := tracker.Version()
	v := oldVersion
	var err error
	var voteInfo, vinfo *bitumjson.GetVoteInfoResult
	...
	if voteInfo == nil {
		if oldVersion == 0 {
			...
			log.Info("No agenda information retreived from bitumd")
			return nil, nil
		}
...
func rciBlocks(voteInfo *bitumjson.GetVoteInfoResult) int64 {
...
func (tracker *VoteTracker) fetchBlocks(voteInfo *bitumjson.GetVoteInfoResult) ([]int32, uint32, error) {
	blocksToRequest := 1
	...
	var block bitumjson.StakeVersions
...
func (tracker *VoteTracker) refreshSVIs(voteInfo *bitumjson.GetVoteInfoResult) (*bitumjson.GetStakeVersionInfoResult, error) {
...
// Once all resources have been retrieved from bitumd, update VoteTracker fields.
func (tracker *VoteTracker) update(voteInfo *bitumjson.GetVoteInfoResult, blocks []int32,
	stakeInfo *bitumjson.GetStakeVersionInfoResult, stakeVersion uint32) {
