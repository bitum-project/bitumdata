...
import (
	...
	"github.com/bitum-project/bitumdata/blockdata"
	"github.com/bitum-project/bitumdata/db/cache"
	"github.com/bitum-project/bitumdata/db/dbtypes"
	"github.com/bitum-project/bitumdata/db/bitumpg"
	"github.com/bitum-project/bitumdata/db/bitumsqlite"
	"github.com/bitum-project/bitumdata/exchanges"
...
func _main(ctx context.Context) error {
	...
	cfg, err := loadConfig()
	if err != nil {
		fmt.Printf("Failed to load bitumdata config: %s\n", err.Error())
		return err
	}
	defer func() {
		if logRotator != nil {
	...
	defer func() {
		if bitumdClient != nil {
			log.Infof("Closing connection to bitumd.")
	...
	curnet, err := bitumdClient.GetCurrentNet()
	if err != nil {
		return fmt.Errorf("Unable to get current network from bitumd: %v", err)
	}
	log.Infof("Connected to bitumd (JSON-RPC API v%s) on %v",
	...
	// StakeDatabase
	stakeDB, stakeDBHeight, err := stakedb.NewStakeDatabase(bitumdClient, activeChain, cfg.DataDir)
	if err != nil {
		log.Errorf("Unable to create stake DB: %v", err)
		if stakeDBHeight >= 0 {
			log.Infof("Attempting to recover stake DB...")
			stakeDB, err = stakedb.LoadAndRecover(bitumdClient, activeChain, cfg.DataDir, stakeDBHeight-288)
	...
	pgDB, err := bitumpg.NewChainDBRPC(chainDB, bitumdClient)
	if err != nil {
		return err
	}
	...
	Heights := func() (dbHeight, nodeHeight, baseDBHeight, pgDBHeight int64, err error) {
		_, nodeHeight, err = bitumdClient.GetBestBlock()
		if err != nil {
	...
	if baseHeight > -1 {
		orphaned, err := rpcutils.OrphanedTipLength(ctx, bitumdClient, baseHeight, baseDB.DB.RetrieveBlockHash)
		if err != nil {
	...
	if auxHeight > -1 {
		orphaned, err := rpcutils.OrphanedTipLength(ctx, bitumdClient, auxHeight, pgDB.BlockHash)
		if err != nil {
	...
	blockHash, nodeHeight, err := bitumdClient.GetBestBlock()
	if err != nil {
		return fmt.Errorf("Unable to get block from node: %v", err)
	}

	block, err := bitumdClient.GetBlockHeader(blockHash)
	if err != nil {
		return fmt.Errorf("unable to fetch the block from the node: %v", err)
	}

	// bestBlockAge is the time since the bitumd best block was mined.
	...
	// expected height of the best block from bitumd now should be this.
	...
	collector := blockdata.NewCollector(bitumdClient, activeChain, stakeDB)
	if collector == nil {
	...
	// transactions that appear in the bitum blockchain. If corrupted data is
	// is found, its deleted pending the data update that restores valid data.
	agendasInstance, err := agendas.NewAgendasDB(bitumdClient,
	...
	mpoolCollector := mempool.NewMempoolDataCollector(bitumdClient, activeChain)
	...
	if displaySyncStatusPage {
		...
		latestDBBlockHash, err := bitumdClient.GetBlockHash(dbHeight)
		if err != nil {
	...
	webMux.With(explore.SyncStatusAPIIntercept).Group(func(r chi.Router) {
		// Mount the bitumdata's REST API.
		r.Mount("/api", apiMux.Mux)
		// Setup and mount the Insight API.
		insightApp := insight.NewInsightApi(bitumdClient, pgDB,
	...
	// Ensure all side chains known by bitumd are also present in the base DB
	// and import them if they are not already there.
	if cfg.ImportSideChains {
		log.Info("Primary DB -> Now retrieving side chain blocks from bitumd...")
	...
	// Ensure all side chains known by bitumd are also present in the auxiliary DB
	// and import them if they are not already there.
	if cfg.ImportSideChains {
		// First identify the side chain blocks that are missing from the DB.
		log.Info("Aux DB -> Retrieving side chain blocks from bitumd...")
		...
		// since bitumsqlite does not track side chain blocks, and stakedb only
		...
		for _, sideChain := range sideChainBlocksToStore {
			...
			for _, hash := range sideChain.Hashes {
				...
				// Store data in the aux (bitumpg) DB.
		...
		log.Infof("Successfully added %d blocks from %d side chains into bitumpg DB.",
	...
	// Monitors that fetch the latest updates from bitumd will be launched next.
	...
	// bitumd notifies of block 200000 while bitumdata has only reached 1000 in
	...
	// already registered for notifications with bitumd but the monitors below are
	// not started, notifications will fill up the channels, only to be
	// processed after sync. This is also incorrect since bitumd might notify of a
...
func waitForSync(ctx context.Context, base chan dbtypes.SyncResult, aux chan dbtypes.SyncResult) (int64, int64, error) {
	...
	if baseRes.Error != nil {
		log.Errorf("bitumsqlite.SyncDBAsync failed at height %d: %v.", baseDBHeight, baseRes.Error)
		requestShutdown()
		auxRes := <-aux
	...
	if auxRes.Error != nil {
		if baseRes.Error != nil {
			log.Error("bitumsqlite.SyncDBAsync AND bitumpg.SyncChainDBAsync "+
		...
		log.Errorf("bitumpg.SyncChainDBAsync failed at height %d.", pgDBHeight)
...
func connectNodeRPC(cfg *config, ntfnHandlers *rpcclient.NotificationHandlers) (*rpcclient.Client, semver.Semver, error) {
	return rpcutils.ConnectNodeRPC(cfg.BitumdServ, cfg.BitumdUser, cfg.BitumdPass,
		cfg.BitumdCert, cfg.DisableDaemonTLS, true, ntfnHandlers)
}
...
func listenAndServeProto(ctx context.Context, wg *sync.WaitGroup, listen, proto string, mux http.Handler) {
	...
	go func() {
		var err error
		if proto == "https" {
			err = server.ListenAndServeTLS("bitumdata.cert", "bitumdata.key")
		} else {
			err = server.ListenAndServe()
		}
		// If the server dies for any reason other than ErrServerClosed (from
		// graceful server.Shutdown), log the error and request bitumdata be
		// shutdown.
