...
func NewMempoolDataCollector(bitumdChainSvr *rpcclient.Client, params *chaincfg.Params) *MempoolDataCollector {
...
func (t *MempoolDataCollector) mempoolTxns() ([]exptypes.MempoolTx, txhelpers.MempoolAddressStore, txhelpers.TxnsStore, error) {
	mempooltxs, err := t.bitumdChainSvr.GetRawMempoolVerbose(bitumjson.GRMAll)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("GetRawMempoolVerbose failed: %v", err)
	}

	blockHash, _, err := t.bitumdChainSvr.GetBestBlock()
	if err != nil {
	...
	for hashStr, tx := range mempooltxs {
		...
		rawtx, err := rpcutils.GetTransactionVerboseByID(t.bitumdChainSvr, hash)
		if err != nil {
			log.Warn(err)
			continue
		}
		...
		txhelpers.TxPrevOutsByAddr(addrMap, txnsStore, msgTx, t.bitumdChainSvr, t.activeChain)
...
func (t *MempoolDataCollector) Collect() (*StakeData, []exptypes.MempoolTx, txhelpers.MempoolAddressStore, txhelpers.TxnsStore, error) {
	...
	mempoolTickets, err := c.GetRawMempoolVerbose(bitumjson.GRMTickets)
	if err != nil {
		return nil, nil, nil, nil, err
	}

	mempoolVotes, err := c.GetRawMempoolVerbose(bitumjson.GRMVotes)
	if err != nil {
		return nil, nil, nil, nil, err
	}
...
func ParseTxns(txs []exptypes.MempoolTx, params *chaincfg.Params, lastBlock *BlockID) *exptypes.MempoolInfo {
	...
	var totalOut, regularTotal, ticketTotal, voteTotal, revTotal bitumutil.Amount
	var likelyMineable bool
	var likelyTotal bitumutil.Amount
	...
	for _, tx := range txs {
		likelyMineable = true
		out, _ := bitumutil.NewAmount(tx.TotalOut) // 0 for invalid amounts
		switch tx.Type {
		case "Ticket":
