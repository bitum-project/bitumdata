...
type MempoolMonitor struct {
	...
	// Incoming data
	newTxIn <-chan *bitumjson.TxRawResult
	// Outgoing message
...
func NewMempoolMonitor(ctx context.Context, collector *MempoolDataCollector,
	...
	newTxInChan <-chan *bitumjson.TxRawResult, signalOuts []chan<- pstypes.HubMessage,
...
func (p *MempoolMonitor) TxHandler(client *rpcclient.Client) {
	...
	for {
		...
		case s, ok := <-p.newTxIn:
			...
			case "Vote":
				// Votes on the next block may be received just prior to bitumdata
