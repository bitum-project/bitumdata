...
// Package notification syncronizes bitumd notifications to any number of
// handlers. Typical use:
// 1. Create a Notifier with NewNotifier.
// 2. Grab bitumd configuration settings with BitumdHandlers.
// 3. Create an bitumd/rpcclient.Client with the settings from step 2.
...
//    providing as an argument the bitumd client created in step 3.
...
// TxHandler is a function that will be called when bitumd reports new mempool
// transactions.
...
// when bitumd reports a new block.
...
// Notifier handles block, tx, and reorg notifications from a bitumd node. Handler
// functions are registered with the Register*Handlers methods. To start the
// Notifier, Listen must be called with a bitumd rpcclient.Client only after all
// handlers are registered.
type Notifier struct {
	ctx  context.Context
	node BITUMDNode
	// The anyQ sequences all bitumd notification in the order they are received.
	anyQ     chan interface{}
	tx       [][]TxHandler
	block    [][]BlockHandler
	reorg    [][]ReorgHandler
...
// BITUMDNode is an interface to wrap a bitumd rpcclient.Client. The interface
// allows testing with a dummy node.
type BITUMDNode interface {
...
// BitumdHandlers creates a set of handlers to be passed to the bitumd
...
func (notifier *Notifier) BitumdHandlers() *rpcclient.NotificationHandlers {
	return &rpcclient.NotificationHandlers{
		...
		// for the mempool monitors to avoid an extra call to bitumd for
		// the tx details
		OnTxAcceptedVerbose: notifier.onTxAcceptedVerbose,
	}
}

// superQueue should be run as a goroutine. The bitumd-registered block and reorg
...
func (notifier *Notifier) onTxAcceptedVerbose(tx *bitumjson.TxRawResult) {
...
// packages don't necessarily need to import bitumd/wire. Handlers registered with
...
// were registered.
func (notifier *Notifier) processTx(tx *bitumjson.TxRawResult) {
	start := time.Now()
...
// signalReorg takes the basic reorganization data from bitumd, determines the two
