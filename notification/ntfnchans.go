...
var NtfnChans struct {
	...
	ConnectChanBitumpgDB                chan *chainhash.Hash
	ReorgChanBitumpgDB                  chan *txhelpers.ReorgData
	...
	RelevantTxMempoolChan             chan *bitumutil.Tx
	NewTxChan                         chan *bitumjson.TxRawResult
...
func MakeNtfnChans() {
	...
	NtfnChans.ConnectChanBitumpgDB = make(chan *chainhash.Hash, blockConnChanBuffer)
	...
	NtfnChans.ReorgChanBitumpgDB = make(chan *txhelpers.ReorgData)
	...
	// 	NtfnChans.RelevantTxMempoolChan = make(chan *bitumutil.Tx, relevantMempoolTxChanBuffer)
	// }

	// New mempool tx chan for general purpose mempool monitor/collector/saver.
	NtfnChans.NewTxChan = make(chan *bitumjson.TxRawResult, newTxChanBuffer)
...
func CloseNtfnChans() {
	...
	if NtfnChans.ConnectChanBitumpgDB != nil {
		close(NtfnChans.ConnectChanBitumpgDB)
	}
	...
	if NtfnChans.ReorgChanBitumpgDB != nil {
		close(NtfnChans.ReorgChanBitumpgDB)
	}
