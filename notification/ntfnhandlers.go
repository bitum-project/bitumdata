...
// RegisterNodeNtfnHandlers registers with bitumd to receive new block,
// transaction and winning ticket notifications.
func RegisterNodeNtfnHandlers(bitumdClient *rpcclient.Client) *ContextualError {
	// Set the node client for use by signalReorg to determine the common
	// ancestor of two chains.
	nodeClient = bitumdClient
	...
	// if len(addresses) > 0 {
	// 	if err = bitumdClient.LoadTxFilter(true, addresses, nil); err != nil {
...
func signalReorg(d ReorgData) {
	...
	// Send reorg data to bitumsqlite's monitor.
	d.WG.Add(1)
	select {
	...
	d.WG.Add(1)
	select {
	case NtfnChans.ReorgChanBitumpgDB <- fullData:
	default:
		d.WG.Done()
	}
	d.WG.Wait()
...
// MakeNodeNtfnHandlers defines the bitumd notification handlers.
func MakeNodeNtfnHandlers() (*rpcclient.NotificationHandlers, *collectionQueue) {
	...
	return &rpcclient.NotificationHandlers{
		...
		// the tx details
		OnTxAcceptedVerbose: func(txDetails *bitumjson.TxRawResult) {
