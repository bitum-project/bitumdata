...
function customizedFormatter (data) {
  ...
  data.series.map((series) => {
    ...
    var l = `<span style="color: ` + series.color + ';"> ' + series.labelHTML
    html = `<span style="color:#2d2d2d;">` + html + `</span>`
    html += '<br>' + series.dashHTML + l + ': ' + (isNaN(series.y) ? '' : series.y + ' BITUM') + '</span> '
  })
  return html
}
...
export default class extends Controller {
  ...
  async connect () {
    ...
    // Parse stimulus data
    var cdata = ctrl.data
    ctrl.bitumAddress = cdata.get('bitumaddress')
  ...
  async showQRCode () {
    ...
    if (this.qrCode) {
      ...
      let qrCodeImg = await QRCode.toDataURL(this.bitumAddress,
        {
  ...
  makeTableUrl (txType, count, offset) {
    return '/addresstable/' + this.bitumAddress + '?txntype=' +
  ...
  async fetchGraphData (chart, bin) {
    ...
    let url = '/api/address/' + ctrl.bitumAddress + '/' + chartKey + '/' + bin
