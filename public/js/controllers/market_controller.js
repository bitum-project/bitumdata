...
const exchangeLinks = {
  binance: 'https://www.binance.com/en/trade/BITUM_BTC',
  bittrex: 'https://bittrex.com/Market/Index?MarketName=BTC-BITUM',
  poloniex: 'https://poloniex.com/exchange#btc_bitum',
  dragonex: 'https://dragonex.io/en-us/trade/index/bitum_btc',
  huobi: 'https://www.hbg.com/en-us/exchange/?s=bitum_btc'
}
...
export default class extends Controller {
  ...
  processVolume (response) {
    ...
    return {
      ...
      labels: ['time', 'volume'],
      xlabel: 'Time',
      ylabel: `Volume (BITUM / ${prettyDurations[settings.bin]})`,
      colors: [chartStroke],
      plotter: Dygraph.Plotters.linePlotter,
      axes: {
        x: {
  ...
  _processXcUpdate (update) {
    ...
    if (update.fiat) { // btc-fiat exchange update
    ...
    } else { // bitum-btc exchange update
      ...
      if (xc.change >= 0) {
        row.arrow.className = 'bitumicon-arrow-up text-green'
      } else {
        row.arrow.className = 'bitumicon-arrow-down text-danger'
      }
    }
