...
function makeMempoolBlock (block) {
  ...
  return makeNode(`<div class="block visible">
                <div class="block-info">
                    ...
                    <div class="mono" style="line-height: 1;">${Math.floor(block.Total)} BITUM</div>
                    <span class="timespan">
...
function makeBlockSummary (blockHeight, totalSent, time) {
  return `<div class="block-info">
                ...
                <div class="mono" style="line-height: 1;">${Math.floor(totalSent)} BITUM</div>
                <span class="timespan">
...
function makeRewardsElement (subsidy, fee, voteCount, rewardTxId) {
  ...
  // const totalBITUM = Math.round(pow + fund + fee);
  const totalBITUM = 1
  return `<div class="block-rewards" style="flex-grow: ${totalBITUM}">
...
function makeVoteElements (votes) {
  ...
  // totalBITUM = Math.round(totalBITUM);
  totalBITUM = 1
  return `<div class="block-votes" style="flex-grow: ${totalBITUM}">
                ${voteElements.join('\n')}
            </div>`
}
...
function makeTicketAndRevocationElements (tickets, revocations, blockHref) {
  ...
  // totalBITUM = Math.round(totalBITUM);
  totalBITUM = 1
  return `<div class="block-tickets" style="flex-grow: ${totalBITUM}">
...
function makeTransactionElements (transactions, blockHref) {
  ...
  // totalBITUM = Math.round(totalBITUM);
  totalBITUM = 1
  return `<div class="block-transactions" style="flex-grow: ${totalBITUM}">
    ...
    this.tooltipTargets.forEach((tooltipElement) => {
      try {
        ...
        if (data.object === 'Vote') {
          ...
          newContent = `<b>${data.object}</b><br>${data.total} BITUM`
        }
