// Copyright (c) 2017-2019, The bitumdata developers
...
type BlockGetter interface {
	...
	BestBlock() (*bitumutil.Block, error)
	Block(chainhash.Hash) (*bitumutil.Block, error)
...
// directly from bitumd via RPC and subsequently update the internal block cache
// with the retrieved block.
type MasterBlockGetter interface {
	BlockGetter
	UpdateToBestBlock() (*bitumutil.Block, error)
	UpdateToNextBlock() (*bitumutil.Block, error)
	UpdateToBlock(height int64) (*bitumutil.Block, error)
}
...
type BlockGate struct {
	...
	blockWithHash map[chainhash.Hash]*bitumutil.Block
...
func NewBlockGate(client BlockFetcher, capacity int) *BlockGate {
	return &BlockGate{
		...
		blockWithHash: make(map[chainhash.Hash]*bitumutil.Block),
...
// NodeHeight gets the chain height from bitumd.
...
func (g *BlockGate) BestBlock() (*bitumutil.Block, error) {
	g.mtx.RLock()
	defer g.mtx.RUnlock()
	var err error
	hash, ok := g.hashAtHeight[g.height]
...
func (g *BlockGate) CachedBlock(hash chainhash.Hash) (*bitumutil.Block, error) {
	g.mtx.RLock()
	defer g.mtx.RUnlock()
	block, ok := g.blockWithHash[hash]
	if !ok {
...
// the event of a cache miss, the block is retrieved from bitumd via RPC.
func (g *BlockGate) Block(hash chainhash.Hash) (*bitumutil.Block, error) {
	...
	// Cache miss. Retrieve from bitumd RPC.
...
func (g *BlockGate) UpdateToBestBlock() (*bitumutil.Block, error) {
	_, height, err := g.client.GetBestBlock()
	if err != nil {
...
// updates the cache.
func (g *BlockGate) UpdateToNextBlock() (*bitumutil.Block, error) {
	g.mtx.Lock()
	height := g.height + 1
	g.mtx.Unlock()
	return g.UpdateToBlock(height)
...
// bitumd, stores it in cache, and signals any waiters. This is the thread-safe
// version of updateToBlock.
func (g *BlockGate) UpdateToBlock(height int64) (*bitumutil.Block, error) {
	g.mtx.Lock()
	defer g.mtx.Unlock()
	return g.updateToBlock(height)
}
...
// updateToBlock gets the block at the specified height on the main chain from
// bitumd. It is not thread-safe. It wrapped by UpdateToBlock for thread-safety,
