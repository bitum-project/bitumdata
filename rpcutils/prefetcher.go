...
type BlockFetcher interface {
	...
	GetBlockHeaderVerbose(hash *chainhash.Hash) (*bitumjson.GetBlockHeaderVerboseResult, error)
}
...
type blockData struct {
	...
	headerResult *bitumjson.GetBlockHeaderVerboseResult
}
...
func (p *BlockPrefetchClient) GetBlockData(hash *chainhash.Hash) (*wire.MsgBlock, *bitumjson.GetBlockHeaderVerboseResult, error) {
	p.Lock()
...
// GetBlockHeaderVerbose retrieves the bitumjson.GetBlockHeaderVerboseResult for
// the block with the specified hash. See GetBlockData for details on how this
// interacts with the prefetcher
func (p *BlockPrefetchClient) GetBlockHeaderVerbose(hash *chainhash.Hash) (*bitumjson.GetBlockHeaderVerboseResult, error) {
...
// becomes "current".
func (p *BlockPrefetchClient) storeNext(msgBlock *wire.MsgBlock, headerResult *bitumjson.GetBlockHeaderVerboseResult) {
	p.current = p.next
	p.next = &blockData{
...
func (p *BlockPrefetchClient) retrieveBlockAndHeaderResult(hash *chainhash.Hash) (*wire.MsgBlock, *bitumjson.GetBlockHeaderVerboseResult, uint32, error) {
