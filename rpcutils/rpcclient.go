...
// Any of the following bitumd RPC API versions are deemed compatible with
// bitumdata.
...
// ConnectNodeRPC attempts to create a new websocket connection to a bitumd node,
...
func ConnectNodeRPC(host, user, pass, cert string, disableTLS, disableReconnect bool,
	...
	var bitumdCerts []byte
	var err error
	var nodeVer semver.Semver
	if !disableTLS {
		bitumdCerts, err = ioutil.ReadFile(cert)
		if err != nil {
		...
		log.Debugf("Attempting to connect to bitumd RPC %s as user %s (no TLS)",
			host, user)
	}
	...
	bitumdClient, err := rpcclient.New(connCfgDaemon, ntfnHdlrs)
	if err != nil {
		return nil, nodeVer, fmt.Errorf("Failed to start bitumd RPC client: %s", err.Error())
	}
	...
	nodeVer = semver.NewSemver(bitumdVer.Major, bitumdVer.Minor, bitumdVer.Patch)

	// Check if the bitumd RPC API version is compatible with bitumdata.
...
// BuildBlockHeaderVerbose creates a *bitumjson.GetBlockHeaderVerboseResult from
...
func BuildBlockHeaderVerbose(header *wire.BlockHeader, params *chaincfg.Params,
	currentHeight int64, nextHash ...string) *bitumjson.GetBlockHeaderVerboseResult {
	if header == nil {
		return nil
	}
	...
	blockHeaderResult := bitumjson.GetBlockHeaderVerboseResult{
		...
		SBits:         bitumutil.Amount(header.SBits).ToCoin(),
...
// GetBlockHeaderVerbose creates a *bitumjson.GetBlockHeaderVerboseResult for the
// block at height idx via an RPC connection to a chain server.
func GetBlockHeaderVerbose(client BlockFetcher, idx int64) *bitumjson.GetBlockHeaderVerboseResult {
...
// GetBlockHeaderVerboseByString creates a *bitumjson.GetBlockHeaderVerboseResult
// for the block specified by hash via an RPC connection to a chain server.
func GetBlockHeaderVerboseByString(client BlockFetcher, hash string) *bitumjson.GetBlockHeaderVerboseResult {
...
// GetBlockVerbose creates a *bitumjson.GetBlockVerboseResult for the block index
// specified by idx via an RPC connection to a chain server.
func GetBlockVerbose(client *rpcclient.Client, idx int64, verboseTx bool) *bitumjson.GetBlockVerboseResult {
	blockhash, err := client.GetBlockHash(idx)
	if err != nil {
		log.Errorf("GetBlockHash(%d) failed: %v", idx, err)
		return nil
...
// GetBlockVerboseByHash creates a *bitumjson.GetBlockVerboseResult for the
// specified block hash via an RPC connection to a chain server.
func GetBlockVerboseByHash(client *rpcclient.Client, hash string, verboseTx bool) *bitumjson.GetBlockVerboseResult {
	blockhash, err := chainhash.NewHashFromStr(hash)
	if err != nil {
...
func GetStakeDiffEstimates(client *rpcclient.Client) *apitypes.StakeDiff {
	...
	stakeDiffEstimates := apitypes.StakeDiff{
		GetStakeDifficultyResult: bitumjson.GetStakeDifficultyResult{
...
func GetBlock(ind int64, client BlockFetcher) (*bitumutil.Block, *chainhash.Hash, error) {
	...
	block := bitumutil.NewBlock(msgBlock)

	return block, blockhash, nil
}

// GetBlockByHash gets the block with the given hash from a chain server.
func GetBlockByHash(blockhash *chainhash.Hash, client BlockFetcher) (*bitumutil.Block, error) {
	msgBlock, err := client.GetBlock(blockhash)
	if err != nil {
...
func SideChains(client *rpcclient.Client) ([]bitumjson.GetChainTipsResult, error) {
...
func sideChainTips(allTips []bitumjson.GetChainTipsResult) (sideTips []bitumjson.GetChainTipsResult) {
	for i := range allTips {
...
// address
func SearchRawTransaction(client *rpcclient.Client, count int, address string) ([]*bitumjson.SearchRawTransactionsResult, error) {
...
// orphaned DB tip whose corresponding block is not known to bitumd will not cause
...
func OrphanedTipLength(ctx context.Context, client BlockHashGetter,
	...
	for {
		...
		// Reorgs are soft-limited to depth 6 by bitumd. More than six blocks without
...
// GetChainWork fetches the bitumjson.BlockHeaderVerbose and returns only the
...
func UnconfirmedTxnsForAddress(client *rpcclient.Client, address string, params *chaincfg.Params) (*txhelpers.AddressOutpoints, int64, error) {
	...
	mempoolTxns, err := client.GetRawMempoolVerbose(bitumjson.GRMAll)
	if err != nil {
	...
	for hash, tx := range mempoolTxns {
		// Transaction details from bitumd
...
func APITransaction(client *rpcclient.Client, txid *chainhash.Hash) (tx *apitypes.Tx, hex string, err error) {
	...
	tx.Vin = make([]bitumjson.Vin, len(txraw.Vin))
