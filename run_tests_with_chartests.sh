...
testrepo () {
  ...
  git clone https://github.com/bitumlabs/bug-free-happiness $TMPDIR
  ...
  # Set up the tests db.
  psql -U postgres -c "DROP DATABASE IF EXISTS bitumdata_mainnet_test"
  psql -U postgres -c "CREATE DATABASE bitumdata_mainnet_test"
...
DOCKER_IMAGE_TAG=bitumdata-golang-builder-$GOVERSION
$DOCKER pull bitum/$DOCKER_IMAGE_TAG
