...
// Copyright (c) 2018, The bitumdata developers
...
type StakeDatabase struct {
	...
	BestNode        *stake.Node
	blkMtx          sync.RWMutex
	blockCache      map[int64]*bitumutil.Block
...
func LoadAndRecover(client *rpcclient.Client, params *chaincfg.Params,
	...
	sDB := &StakeDatabase{
		params:          params,
		NodeClient:      client,
		blockCache:      make(map[int64]*bitumutil.Block),
...
func (db *StakeDatabase) PopulateLiveTicketCache() error {
	var err error
	// Live tickets from bitumdata's stake Node's perspective
...
func (db *StakeDatabase) BlockCached(ind int64) (*bitumutil.Block, bool) {
	db.blkMtx.RLock()
...
func (db *StakeDatabase) block(ind int64) (*bitumutil.Block, bool) {
...
func (db *StakeDatabase) ConnectBlockHash(hash *chainhash.Hash) (*bitumutil.Block, error) {
...
func (db *StakeDatabase) ConnectBlock(block *bitumutil.Block) error {
	height := block.Height()
...
func (db *StakeDatabase) connectBlock(block *bitumutil.Block, spent []chainhash.Hash,
...
func (db *StakeDatabase) Open(dbName string) error {
	...
	if err != nil {
		if strings.Contains(err.Error(), "resource temporarily unavailable") ||
			...
			return fmt.Errorf("Stake DB already opened. bitumdata running?")
		}
...
func (db *StakeDatabase) makePoolInfo(poolValue, poolSize int64,
	...
	poolCoin := bitumutil.Amount(poolValue).ToCoin()
	valAvg := 0.0
	if poolSize > 0 {
...
// DBTipBlock gets the bitumutil.Block for the current best block in the stake
// database. It used DBState to get the best block hash, and the node RPC client
// to get the block itself.
func (db *StakeDatabase) DBTipBlock() (*bitumutil.Block, error) {
	_, hash, err := db.DBState()
	if err != nil {
		return nil, err
	}
...
// DBPrevBlock gets the bitumutil.Block for the previous best block in the stake
// database. It used DBState to get the best block hash, and the node RPC client
// to get the block itself.
func (db *StakeDatabase) DBPrevBlock() (*bitumutil.Block, error) {
	_, hash, err := db.DBState()
	if err != nil {
		return nil, err
	}
...
func (db *StakeDatabase) getBlock(hash *chainhash.Hash) (*bitumutil.Block, error) {
	msgBlock, err := db.NodeClient.GetBlock(hash)
	if err == nil {
		return bitumutil.NewBlock(msgBlock), nil
	}
	return nil, err
}
