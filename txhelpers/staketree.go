...
import (
	...
	_ "github.com/bitum-project/bitumd/database/ffldb" // init the ffldb driver
...
func BuildStakeTree(blocks map[int64]*bitumutil.Block, netParams *chaincfg.Params,
...
func TicketsInBlock(bl *bitumutil.Block) ([]chainhash.Hash, []*wire.MsgTx) {
...
func TicketTxnsInBlock(bl *bitumutil.Block) ([]chainhash.Hash, []*bitumutil.Tx) {
	tickets := make([]chainhash.Hash, 0)
	ticketTxns := make([]*bitumutil.Tx, 0)
	for _, stx := range bl.STransactions() {
		if stake.DetermineTxType(stx.MsgTx()) == stake.TxTypeSStx {
			h := stx.Hash()
			tickets = append(tickets, *h)
...
func TicketsSpentInBlock(bl *bitumutil.Block) []chainhash.Hash {
	tickets := make([]chainhash.Hash, 0)
	for _, stx := range bl.STransactions() {
...
func VotesInBlock(bl *bitumutil.Block) []chainhash.Hash {
...
func RevokedTicketsInBlock(bl *bitumutil.Block) []chainhash.Hash {
	tickets := make([]chainhash.Hash, 0)
	for _, stx := range bl.STransactions() {
