...
type RawTransactionGetter interface {
	GetRawTransaction(txHash *chainhash.Hash) (*bitumutil.Tx, error)
}
...
type VerboseTransactionGetter interface {
	GetRawTransactionVerbose(txHash *chainhash.Hash) (*bitumjson.TxRawResult, error)
...
// TxhashInSlice searches a slice of *bitumutil.Tx for a transaction with the hash
// txHash. If found, it returns the corresponding *Tx, otherwise nil.
func TxhashInSlice(txs []*bitumutil.Tx, txHash *chainhash.Hash) *bitumutil.Tx {
	if len(txs) < 1 {
		return nil
	}
...
func IncludesStakeTx(txHash *chainhash.Hash, block *bitumutil.Block) (int, int8) {
...
func IncludesTx(txHash *chainhash.Hash, block *bitumutil.Block) (int, int8) {
...
func BlockConsumesOutpointWithAddresses(block *bitumutil.Block, addrs map[string]TxAction,
	c RawTransactionGetter, params *chaincfg.Params) map[string][]*bitumutil.Tx {
	addrMap := make(map[string][]*bitumutil.Tx)

	checkForOutpointAddr := func(blockTxs []*bitumutil.Tx) {
					...
					for _, txAddr := range txAddrs {
						...
						if _, ok := addrs[addrstr]; ok {
							if addrMap[addrstr] == nil {
								addrMap[addrstr] = make([]*bitumutil.Tx, 0)
							}
...
// specified addresses, and creates a map of addresses to a slice of bitumutil.Tx
// involving the address.
func BlockReceivesToAddresses(block *bitumutil.Block, addrs map[string]TxAction,
	params *chaincfg.Params) map[string][]*bitumutil.Tx {
	addrMap := make(map[string][]*bitumutil.Tx)

	checkForAddrOut := func(blockTxs []*bitumutil.Tx) {
		for _, tx := range blockTxs {
			// Check the addresses associated with the PkScript of each TxOut
				...
				for _, txAddr := range txOutAddrs {
					...
					if _, ok := addrs[addrstr]; ok {
						if _, gotSlice := addrMap[addrstr]; !gotSlice {
							addrMap[addrstr] = make([]*bitumutil.Tx, 0) // nil
						}
...
func OutPointAddresses(outPoint *wire.OutPoint, c RawTransactionGetter,
	params *chaincfg.Params) ([]string, bitumutil.Amount, error) {
	...
	value := bitumutil.Amount(txOut.Value)
...
func MedianAmount(s []bitumutil.Amount) bitumutil.Amount {
	if len(s) == 0 {
		return 0
	}

	sort.Sort(bitumutil.AmountSorter(s))
...
// MedianCoin gets the median BITUM from a slice of float64s
...
func SSTXInBlock(block *bitumutil.Block) []*bitumutil.Tx {
...
func SSGenVoteBlockValid(msgTx *wire.MsgTx) (BlockValidation, uint16, error) {
	...
	blockValid := BlockValidation{
		...
		Validity: bitumutil.IsFlagSet16(ssGenVoteBits, bitumutil.BlockValid),
	}
	return blockValid, ssGenVoteBits, nil
}

// VoteBitsInBlock returns a list of vote bits for the votes in a block
func VoteBitsInBlock(block *bitumutil.Block) []stake.VoteVersionTuple {
...
// specified block.
func FeeInfoBlock(block *bitumutil.Block) *bitumjson.FeeInfoBlock {
	feeInfo := new(bitumjson.FeeInfoBlock)
	...
	for it, msgTx := range sstxMsgTxns {
		...
		fee := bitumutil.Amount(amtIn - amtOut).ToCoin()
		if fee < minFee {
			minFee = fee
		}
		if fee > maxFee {
...
func FeeRateInfoBlock(block *bitumutil.Block) *bitumjson.FeeInfoBlock {
	feeInfo := new(bitumjson.FeeInfoBlock)
	_, sstxMsgTxns := TicketsInBlock(block)
	...
	for it, msgTx := range sstxMsgTxns {
		...
		fee := bitumutil.Amount(1000*(amtIn-amtOut)).ToCoin() / float64(msgTx.SerializeSize())
		if fee < minFee {
			minFee = fee
		}
		if fee > maxFee {
...
func TxFee(msgTx *wire.MsgTx) bitumutil.Amount {
	var amtIn int64
	for iv := range msgTx.TxIn {
		amtIn += msgTx.TxIn[iv].ValueIn
	}
	...
	return bitumutil.Amount(amtIn - amtOut)
}

// TxFeeRate computes and returns the total fee in atoms and fee rate in
// atoms/kB for a given tx.
func TxFeeRate(msgTx *wire.MsgTx) (bitumutil.Amount, bitumutil.Amount) {
	var amtIn int64
	for iv := range msgTx.TxIn {
		amtIn += msgTx.TxIn[iv].ValueIn
	}
	...
	return bitumutil.Amount(amtIn - amtOut), bitumutil.Amount(FeeRate(amtIn, amtOut, txSize))
}
...
func TotalOutFromMsgTx(msgTx *wire.MsgTx) bitumutil.Amount {
	var amtOut int64
	for _, v := range msgTx.TxOut {
		amtOut += v.Value
	}
	return bitumutil.Amount(amtOut)
}

// TotalVout computes the total value of a slice of bitumjson.Vout
func TotalVout(vouts []bitumjson.Vout) bitumutil.Amount {
	var total bitumutil.Amount
	for _, v := range vouts {
		a, err := bitumutil.NewAmount(v.Value)
		if err != nil {
			continue
		}
		total += a
...
// hash) address. See https://github.com/bitum-project/bitumdata/issues/358 for details.
func IsZeroHashP2PHKAddress(checkAddressString string, params *chaincfg.Params) bool {
	...
	address, err := bitumutil.NewAddressPubKeyHash(zeroed[:], params, 0)
	if err != nil {
		return false
	}
...
// network.
func ValidateNetworkAddress(address bitumutil.Address, p *chaincfg.Params) bool {
...
// string. Initially, decoding as a Bitum address is attempted. If it fails to
// decode, AddressErrorDecodeFailed is returned with AddressTypeUnknown.
// If the address decoded successfully as a Bitum address, it is checked
...
// address type.
func AddressValidation(address string, params *chaincfg.Params) (bitumutil.Address, AddressType, AddressError) {
	...
	// Detect when an address belonging to a different Bitum network.
	if !ValidateNetworkAddress(addr, params) {
		return addr, AddressTypeUnknown, AddressErrorWrongNet
	}

	// Determine address type for this valid Bitum address. Ignore the error
