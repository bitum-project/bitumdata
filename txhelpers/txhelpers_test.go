...
type TxGetter struct {
	txLookup map[chainhash.Hash]*bitumutil.Tx
}

func (t TxGetter) GetRawTransaction(txHash *chainhash.Hash) (*bitumutil.Tx, error) {
...
func LoadTestBlockAndSSTX(t *testing.T) (*bitumutil.Block, []*bitumutil.Tx) {
	// Load block data
	...
	block, err := bitumutil.NewBlockFromReader(blockTestFile)
	if err != nil {
	...
	allTxRead := make([]*bitumutil.Tx, numSSTX)
	for i := range allTxRead {
		...
		allTxRead[i], err = bitumutil.NewTxFromReader(txFile)
		if err != nil {
			t.Fatal(err)
		}
...
func TestFeeRateInfoBlock(t *testing.T) {
	...
	fibExpected := bitumjson.FeeInfoBlock{
		Height: 138883,
		Number: 20,
...
func TxToWriter(tx *bitumutil.Tx, w io.Writer) error {
	msgTx := tx.MsgTx()
...
func ConnectNodeRPC(host, user, pass, cert string, disableTLS bool) (*rpcclient.Client, semver.Semver, error) {
	...
	bitumdClient, err := rpcclient.New(connCfgDaemon, nil)
	if err != nil {
...
func TestIsZeroHashP2PHKAddress(t *testing.T) {
	...
	testIsZeroHashP2PHKAddress(t, testnetDummy, &chaincfg.TestNetParams, positiveTest)
	...
	testIsZeroHashP2PHKAddress(t, simnetDummy, &chaincfg.TestNetParams, negativeTest)
	...
	testIsZeroHashP2PHKAddress(t, "", &chaincfg.TestNetParams, negativeTest)
}
